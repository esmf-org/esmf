%  $Id: ESMF_builddetail.tex,v 1.10 2003/11/12 22:39:02 svasquez Exp $
...
absoft, intel, lahey or nag.  If the value is default or the
ESMF\_COMPILER is left unset, then the default compiler will be
used. Exceptions for default values: On Linux machines the default
value is lahey and on Darwin machines it is absoft.
...
\begin{verbatim}
    build_config/ESMF_ARCH.ESMF_COMPILER.ESMF_PREC.ESMF_SITE/conf.h
\end{verbatim}
...
makefile fragment and conf.h files.  The new makefile fragment file has to follow the naming
...
Porting to a new machine will require a new conf.h as well. 
New conf.h files have to define the same machine attributes as
existing conf.h files. Example:

\begin{verbatim}
      build_config/Linux.pgi.mysite/build_rules.mk
      build_config/Linux.pgi.mysite/conf.h
\end{verbatim}
