// $Id: ESMCI_Test.C,v 1.9 2008/07/30 22:17:32 rosalind Exp $
//
...
static const char *const version = "$Id: ESMCI_Test.C,v 1.9 2008/07/30 22:17:32 rosalind Exp $";
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
  ...
  if (name == NULL || result == NULL || failMsg == NULL || file == NULL) {
    sprintf(msgbuf, "FAIL %s, line %d, null pointer(s) passed to "
      "ESMCI::Test()\n", __FILE__, __LINE__);
    whichLog->WriteLog(msgbuf, ESMC_LOG_INFO);
    if (!only)
