! $Id: user_model1.F90,v 1.10 2011/06/27 22:31:09 rokuingh Exp $
!
...
module user_model1
  ...
  public userm1__setvm, userm1_register
...
!   !   private to the module.
  ...
  subroutine userm1__setvm(comp, rc)
  ...
  subroutine userm1_register(comp, rc)
    ...
    call ESMF_GridCompSetEntryPoint(comp, method=ESMF_METHOD_INITIALIZE, &
      userRoutine=user_init, rc=rc)
    if (rc/=ESMF_SUCCESS) return ! bail out
    call ESMF_GridCompSetEntryPoint(comp, method=ESMF_METHOD_RUN, &
      userRoutine=user_run, rc=rc)
    if (rc/=ESMF_SUCCESS) return ! bail out
    call ESMF_GridCompSetEntryPoint(comp, method=ESMF_METHOD_FINALIZE, &
...
subroutine userm1_setvm(comp, rc)
  ...
  type(ESMF_GridComp) :: comp
  integer, intent(out) :: rc
  call userm1__setvm(comp, rc)
end subroutine

subroutine userm1_reg(comp, rc)
  use ESMF_Mod
  use user_model1
  type(ESMF_GridComp) :: comp
  integer, intent(out) :: rc
  call userm1_register(comp, rc)
end subroutine
