! $Id: ESMF_BundleRedistBlk2BlkSTest.F90,v 1.1.2.2 2006/08/30 23:04:17 samsoncheung Exp $
!
...
! Grid has two Bundle created from it, the first as the source for the test
! and the second for the final results.  The second Grid has a single Bundle
...
!-------------------------------------------------------------------------
     ...
     print *, "System Test BundleRedistBlk2Blk."
!
!-------------------------------------------------------------------------
!-------------------------------------------------------------------------
!  Create section
...
!
     ...
     ! distributed Bundle to the another distributed Bundle and the second
     ! from the 2nd regularly distributed Bundle back to the 1st
     ! distributed Bundle
...
!   Print result
    ...
    ! distributed to the Bundle and back again are the same
    match    = .true.
    miscount = 0
    do j   = 1,localCounts(2)
      do i = 1,localCounts(1)
...
!-------------------------------------------------------------------------
!-------------------------------------------------------------------------
20    print *, "System Test BundleRedistBlk2Blk complete."

    if ((myPet .eq. 0) .or. (status .ne. ESMF_SUCCESS)) then
      write(failMsg, *)  "Redistribution back not same as original"
      write(testname, *) "System Test BundleRedistBlk2Blk: Bundle Redistribute"
