! $Id: FlowArraysMod.F90,v 1.14 2007/06/22 23:21:58 cdeluca Exp $
!
!-------------------------------------------------------------------------
!BOP
!
...
!  including ESMF\_Fields, ESMF\_InternGrids,  and ESMF\_Arrays.
!
!EOP
!
...
!-------------------------------------------------------------------------
      ...
      subroutine FlowArraysAlloc(interngrid, rc)
...
!\end{verbatim}
!     Here we are getting a pointer to the data inside the Field and calling it
...
!EOP
      ...
      field_de   = ESMF_FieldCreate(interngrid, arrayspec, horzRelloc=ESMF_CELL_CENTER, &
...
!
      lowerindex = lbound(de)
      upperindex = ubound(de)
