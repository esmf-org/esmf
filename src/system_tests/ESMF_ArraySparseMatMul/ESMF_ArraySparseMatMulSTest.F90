! $Id: ESMF_ArraySparseMatMulSTest.F90,v 1.6 2007/06/22 23:21:51 cdeluca Exp $
!
!-------------------------------------------------------------------------
...
!    Two interngridded components and one coupler component, one-way coupling.
!
!    First interngridded component runs on 4 PETs and defines a 2D source Array 
!    100x150. Second interngridded component defines a destination Array also 
!    100x150 but runs on only 2 PETs. Both interngridded components use DELayouts 
...
!    destination Array.
!    
!    Finally the second interngridded component compares the data stored in the
...
!
  ...
  comp2 = ESMF_InternGridCompCreate(name=cname2, petList=(/4,5/), rc=rc)
  print *, "Created component ", trim(cname2), "rc =", rc
  if (rc .ne. ESMF_SUCCESS) goto 10
  !  call ESMF_InternGridCompPrint(comp2, "", rc)
...
!-------------------------------------------------------------------------
  ...
  if (rc .ne. ESMF_SUCCESS) goto 10
  call ESMF_InternGridCompInitialize(comp2, importState=c2imp, rc=rc)
...
!-------------------------------------------------------------------------
  ...
  call ESMF_InternGridCompRun(comp2, importState=c2imp, rc=rc)
...
!-------------------------------------------------------------------------
  ...
  call ESMF_InternGridCompFinalize(comp2, importState=c2imp, rc=rc)
