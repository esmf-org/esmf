! $Id: ESMF_FieldExclSTest.F90,v 1.29 2007/06/23 04:01:24 cdeluca Exp $
!
...
!
    ...
    cname1 = "user model 1"
    comp1 = ESMF_IGridCompCreate(name=cname1, petList=(/ 6,2,4,0 /), rc=rc)
    if (rc .ne. ESMF_SUCCESS) goto 10
    !print *, "Created component ", trim(cname1), "rc =", rc

    cname2 = "user model 2"
    comp2 = ESMF_IGridCompCreate(name=cname2, petList=(/ 5,1,3 /), rc=rc)
    if (rc .ne. ESMF_SUCCESS) goto 10
...
!-------------------------------------------------------------------------
    ...
    do while (.not. ESMF_ClockIsStopTime(clock, rc))
      ...
      ! Uncomment the following call to ESMF_IGridCompWait() to sequentialize
      ! comp1 and comp2. The following ESMF_IGridCompWait() call will block
      ! all PETs until comp2 has returned. Consequently comp1 will not be
      ! run until comp2 has returned.
      !call ESMF_IGridCompWait(comp2, blockingflag=ESMF_BLOCKING, rc=rc)
      ...
      ! Uncomment the following calls to ESMF_IGridCompWait() to sequentialize
      ! comp1, comp2 and the coupler. The following ESMF_IGridCompWait() calls 
      ...
      ! returned.
      !call ESMF_IGridCompWait(comp1, blockingflag=ESMF_BLOCKING, rc=rc)
