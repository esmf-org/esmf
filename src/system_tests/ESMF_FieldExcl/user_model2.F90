! $Id: user_model2.F90,v 1.28 2009/03/23 20:40:48 theurich Exp $
!
...
!   !   private to the module.
    ...
    subroutine userm2_register(comp, rc)
        ...
        call ESMF_GridCompSetEntryPoint(comp, ESMF_SETINIT, user_init, rc=localrc)
        call ESMF_GridCompSetEntryPoint(comp, ESMF_SETRUN, user_run, rc=localrc)
        call ESMF_GridCompSetEntryPoint(comp, ESMF_SETFINAL, user_final, rc=localrc)
...
!   ! Local variables
      ...
      humidity2 = ESMF_FieldCreate(igrid1, arrayspec, &
      ...
      ! create the second igrid and field for use in the redist part of the test
      counts(1) = 800
      counts(2) = 40
      min(1) = 0.0
      max(1) = 60.0
      ...
      igrid2 = ESMF_IGridCreateHorzXYUni(counts=counts, &
                                       ...
                                       horzStagger=horz_stagger, &
                                       name="redist igrid", rc=status)
      if (status .ne. ESMF_SUCCESS) goto 10
      call ESMF_IGridDistribute(igrid2, delayout=delayout, &
                               rc=status)

      ! Create the field and have it create the array internally
      pressure2 = ESMF_FieldCreate(igrid2, arrayspec, &
