! $Id: ESMF_ArrayRedistOpenMPSTest.F90,v 1.1 2009/05/27 22:35:55 svasquez Exp $
!
!-------------------------------------------------------------------------
...
!    To verify the openmp functionallity a test a 4 element array is created
!    where each openmp thread stores its thread number in its element location.
!
...
#include "ESMF.h"
  ...
  integer, dimension(4) :: openmp_array
  ...
  integer :: omp_get_num_threads,  omp_get_thread_num, tid, i
...
!
  ...
  if (ESMF_LogMsgFoundError(localrc, ESMF_ERR_PASSTHRU, &
    ...
    ! Populate the openmp test array.
!$omp parallel
        tid = omp_get_thread_num()
	openmp_array(tid)=tid
!$omp end parallel
  ! Verify openmp ran successfully
  do i = 0,3
	if (openmp_array(i)/=i)  &
	call ESMF_Finalize(rc=rc, terminationflag=ESMF_ABORT)
  end do
