! $Id: ESMF_VectorStorageSTest.F90,v 1.7 2005/03/03 22:26:35 nscollins Exp $
!
...
! System test VectorStorage.
!
! This system test checks the functionality of the vector grid distribution
! routines by redistributing data from one Field distributed in the normal
! block structure to another Field that has been distributed as a vector
...
! structure and the other with a semi-random vector distribution.  The first
...
!-------------------------------------------------------------------------
     ...
     print *, "System Test VectorStorage."
!
!-------------------------------------------------------------------------
!-------------------------------------------------------------------------
!  Create section
...
!
     ...
     ! block style and the second is distributed in vector style
     ...
     ! distributed Field to the vector Field and the second from the
...
!   Print result
    ...
    ! distributed to the vector Field and back again are the same
    match    = .true.
    miscount = 0
    do j   = 1,localCounts(2)
      do i = 1,localCounts(1)
...
!-------------------------------------------------------------------------
!-------------------------------------------------------------------------
20    print *, "System Test VectorStorage complete."

    if ((myPet .eq. 0) .or. (status .ne. ESMF_SUCCESS)) then
      write(failMsg, *)  "Redistribution back not same as original"
      write(testname, *) "System Test VectorStorage: Field Redistribute"
      ...
      if ((miscount.eq.0) .and. (status .eq. ESMF_SUCCESS)) then
        ...
        "Data redistribution does not match expected values, or error code set ",status 
      endif
      write(0, *) ""
