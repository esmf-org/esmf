% $Id: ESMF_superoverview.tex,v 1.21 2004/12/30 19:16:01 cdeluca Exp $
%
...
\label{fig:GEOS5}
\scalebox{1.0}{\includegraphics{ESMF_GEOS5.eps}}
\end{figure}
\end{center}
...
\label{fig:serial}
\scalebox{1.0}{\includegraphics{ESMF_serial.eps}}
\end{figure}
\end{center}
...
\scalebox{1.0}{\includegraphics{ESMF_concurrent.eps}}
\end{figure}
\end{center}
...
an ESMF-supplied function called {\bf reconcile} creates proxy
...
to describe
the remote source for data being moved to the current PET,
or as a remote destination for local data being moved from this PET.

\begin{center}
\begin{figure}
\caption{An {\tt ESMF\_StateReconcile()} call creates proxy 
objects for use in communication calls.  Such a call would normally
be made during Coupler initialization.}
\label{fig:reconcile}
\scalebox{1.0}{\includegraphics{ESMF_reconcile.eps}}
\end{figure}
\end{center}
