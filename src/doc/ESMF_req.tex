...
processor counts, or different parallelism options.
...
The \emph{default} execution mode will allow an
ESMF-based model to return bitwise identical results from the same
executable at the same processor count, with the same parallelism
options.

ESMF must provide an optional execution mode where the same
executable, run on a different number of processors under the same
parallelization mode, produces bitwise-reproducible results. This mode
is not subject to the performance requirements above.
\begin{reqlist}
{\bf Priority:} 1.\\
{\bf Source:} all codes. \\
{\bf Status:} Proposed. \\
