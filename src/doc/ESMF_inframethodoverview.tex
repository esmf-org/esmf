% $Id: ESMF_inframethodoverview.tex,v 1.10 2005/11/29 20:57:09 nscollins Exp $
...
copy, an MPI call, or something else.  The ESMF goal of providing
performance portability means the Framework will select the
...
{\tt ESMC\_XPacket}, {\tt ESMC\_CommTable}, and {\tt ESMC_RTable};
...
Route for a Bundle, then the RouteHandle can contain a list of Routes,
one for each Field in the Bundle, and for Regrid use, a list of additional
Routes instead of a single Route.  There is also a flag to indicate whether
a single Route is applicable to all Fields in a Bundle or whether there
are multiple Routes.
