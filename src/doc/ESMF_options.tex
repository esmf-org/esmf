...
\item [ESMF\_FILEFORMAT\_SHAPEFILE] Shapefile file format. This format is a vector data file format commonly used for geospatial analysis. 
...
\item [ESMF\_REGRIDMETHOD\_BINNING]
  Binning regridding. For this regrid method, values located at each source point are mapped to the destination cell that contains the point. A given destination cell
  may receive input from multiple source points, but no source point will go to more than one destination cell. The value in a destination cell is the sum of all the source values
  mapped to it. 
\end{description}
