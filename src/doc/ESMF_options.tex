% $Id: ESMF_options.tex,v 1.87 2012/09/28 23:01:52 theurich Exp $
...
{\sf DESCRIPTION:\\}
{\tt status} is similar to the same keyword in the Fortran {\tt open}
statement. It is used by by I/O output functions such as
{\tt ESMF\_ArrayWrite} and {\tt ESMF\_FieldBundleWrite}. It works in
conjunction with other optional inputs from those functions, {\tt iofmt}
and {\tt overwrite}. See the documentation of those functions for details.

The type of this flag is:

{\tt type(ESMF\_FileStatusFlag)}
...
\item [ESMF\_FILESTATUS\_OLD]
      The file must exist. New data can be added to a file if
      it is in a NetCDF format, however, existing data will not be
      overwritten unless overwrite permission is granted (through an
      additional {\tt overwrite} flag.
      If {\tt iofmt} is specified as {\tt ESMF\_IOFMT\_BIN},
      the write will only succeed if {\tt overwrite} is .true.
...
\item [ESMF\_TERMORDER\_STRICT]
         The source terms are in strict ascending order according to
         their source sequence index.
\item [ESMF\_TERMORDER\_SRCPET]
         The source terms are ordered first according to their location on 
         the source side PETs. The term order within each source PET is given 
         by the source term sequence index.
