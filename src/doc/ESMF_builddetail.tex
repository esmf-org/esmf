%  $Id: ESMF_builddetail.tex,v 1.1 2005/05/06 20:10:39 nscollins Exp $

% there is a separate copy of this under build/doc which is
% now used only by the implementation report.  this was separated
% out because it replicated information which was in the ESMF_install file.
...
name.  ESMF\_SITE, if set, is generally the current machine name,
the location, or the organization (e.g. mit, cola).  If there are no site
specific files for a particular architecture, then ESMF\_COMPILER and
ESMF\_SITE will be set to {\tt default}.  Examples:
...
Where ESMF\_ARCH, ESMF\_COMPILER, and ESMF\_SITE are
...
need to be customized to support a new architecture.
