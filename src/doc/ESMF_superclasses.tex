...
coupler initialization method creates input and export packets, which at this
stage may include empty buffers for packing and unpacking data.  It returns
transforms (in this case, perhaps a combined unit change, regrid and communication 
...
of the atmosphere and ocean components, which check that their import states are 
valid as they begin executing concurrently on
different {\tt DE} sets.  Arguments to these run methods include a run time interval
and the transform provided by the coupler, which contains different
tasks for the two components.  At an appropriate time the
...
DEs in the example, the transform could be staged on a separate set of coupler
DEs.)  The atmosphere calls a state transform complete call with the transform
...
timestep, and import states are validated as the timestep begins.
