% $Id: ESMF_RegridWeightGen.tex,v 1.29 2011/01/24 20:15:45 svasquez Exp $
...
the bilinear, patch, or first order conservative methods decribed below. This application 
can do regrid weight generation from a global source grid to a global destination grid, or from a 
regional source grid to a regional desitination grid.
It assumes that the source and destination grids are on a sphere and that the coordinates given in 
the files are latitude and longitude values. This file based regrid weight generation application is fully parallel. This application is used in the 
...
This application requires NetCDF libary to run.  To compile ESMF with NetCDF, the following
ESMF environment variables need to be set:  {\tt ESMF\_NETCDF}, {\tt ESMF\_NETCDF\_INCLUDE}
and {\tt ESMF\_NETCDF\_LIBPATH}.  Please refer to \ref{sec:netcdf} for the proper values of
these environment variables.  
In addition, ESMF has to be compiled with LAPACK library to generate the patch regridding weights.  
To compile ESMF with LAPACK, the following ESMF environment variables has to be set:
{\tt ESMF\_LAPACK}, {\tt ESMF\_LAPACK\_LIBPATH}, and {\tt ESMF\_LAPACK\_LIBS}. Please refer to
 \ref{sec:lapack} for the proper values of these set of environment variables.
...
({\tt ESMF\_STAGGERLOC\_CORNER}), for conservative regridding. The 2D coordinates are mapped
into 3D Cartesian coordiantes by setting the {\tt regridScheme} flag to {\tt ESMF\_REGRID\_SCHEME\_FULL3D} while calling 
the method, {\tt ESMF\_FieldRegridStore()}.   The method
