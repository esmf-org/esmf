...
\item {\bf ArrayBundle}  An ArrayBundle is a collection of Arrays
distributed in the same manner.  It is useful for performing optimized
...
useful for performing optimized collective data operations and
communications.
...
can be nested so that simpler components and contained within more
complex ones.
...
the application.   The main spatial classes in ESMF, in order of
...
the computing environment, to decide how they want to use the resources associated with the PETs they've received.
...
are connected - with the PETs in a VM.  DEs are not
...
These are:
\begin{itemize}
\item {\bf Attributes}, for storing metadata about Field, FieldBundles,
...
The calendar management utility is a popular place to start.
...
by wrapping model data in either ESMF Arrays or Fields.
These Arrays and Fields are then added to the Import and
