...
\begin{verbatim}
call ESMF_<class><method>(class_instance, arg1, arg2, ..., rc)
\end{verbatim}
...
classes.  The {\tt Create()} method allocates memory 
...
\item {\tt Set()} and {\tt Get()}, for setting and retrieving a 
particular item or flag in a class.  In general, these methods are
overloaded for all cases where the item can be described with a
...
for all cases in this way would be overwhelming, we define
specific {\tt Set<Something>} and {\tt Get<Something>} interfaces.

\item {\tt Add()}, {\tt Get()}, and {\tt Remove()} for manipulating 
items that can be appended or inserted into a list of like
items within a class.

\item {\tt Print()}, for printing the contents of a class to 
standard out.  This method is mainly intended for debugging.

\item {\tt ReadRestart()} and {\tt WriteRestart}, for saving 
the contents of a class and restoring it exactly.

\item {\tt Validate()}, for determining whether a class is 
internally consistent.  For example, Validate can check whether
the Array and Grid associated with a Field are consistent.
