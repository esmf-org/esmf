...
All objects have a Print method to print out internal information, usually
for debugging purposes.  All print methods try to support a common
...
\item{\bf "full"}
   Print in excruiciating detail, as much information as possible.
\item{\bf "object only"}
...
{\tt call ESMF_XXXPrint(object, "brief, object only", rc) }
{\tt call ESMF_XXXPrint(object, "full, label=Temperature", rc) }
...
All objects have a Validate method to verify the object has been
initialized and that it is internally consistent.
All print methods try to support a common set of options, including:
...
\item{\bf "brief"}
   ...
   and DataMaps).  This is the default if no option string is specified.
\item{\bf "full"}
   Validate in excruiciating detail, may be slow.
\item{\bf "object only"}
...
{\tt call ESMF_XXXValidate(object, "brief, object only", rc) }
