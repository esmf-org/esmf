% $Id: ESMF_testing.tex,v 1.23.2.3 2008/07/17 00:12:53 theurich Exp $
...
The same issues discussed above, of how to lauch an MPI application, apply 
directly. 
...
be able to lauch MPI applications across {\tt N} processes by calling
...
"on the fly" and submit this batch script to the queueing software. The script
...
Users working on a system with a parallel execution environment not covered by
any of the existing scripts are encouraged to use the closes match and customize
it to their situation. Pointing {\tt ESMF\_MPIRUN} to a specific file will 
override any default settings. The best way to see how the existing scripts are
used on the supported platforms is to go to the 
\htmladdnormallink{http://www.esmf.ucar.edu/download/platforms/}
{"Supported Platforms"} web page and follow the link for the platform of
interest. Each test report contains the output of {\tt gmake info}, which
lists the settings of the {\tt ESMF\_MPIxxx} environment variables.
