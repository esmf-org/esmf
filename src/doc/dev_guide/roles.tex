%===============================================================================
% CVS $Id: roles.tex,v 1.8 2006/10/24 21:50:56 murphysj Exp $
...
\item open tickets (see section \autoref{tracking_tools} for types) and their status
\end{itemize}

Code {\bf Advocates} should do the following for their code:
\begin{itemize}
\item Touch base periodically and find out how things are going
\item Coordinate the resolution of all tickets (see section \autoref{tracking_tools} for types)
\item Be prepared to brief the status of all requests and issues
\item Update or ensure the update of related tickets (see Section \autoref {usr_support} for details on
...
{\bf Code Points of Contact} should not take advantage of their {\bf Advocates}. If you have an issue, 
...
\autoref{tracking_tools} for list). They are expected to be sufficiently familiar with the issue and the
code involved so as to not unnecessarily pester the customer with requests for information (e.g. the
information is contained in previous tickets from the same customer). See section \autoref{usr_support} for
