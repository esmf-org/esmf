% CVS $Id: code_conv_language.tex,v 1.3 2006/12/13 20:19:07 cdeluca Exp $

\subsection{Interlanguage Coding Conventions}
...
on the ESMF website navigation bar.  These
...
\subsection{Fortran Coding Standard}
...
The conventions also assume the use of the automatic dependency
generator
\htmladdnormallink{mkmf}{http://www.gfdl.gov/\~{}vb/mkmf.html} and the
...
\item Use is restricted to short code sections (a useful rule of thumb
  is that an \texttt{\#ifdef} and the matching \texttt{\#endif} must
...
We will identify a consistent strategy for handling cases in which the
built-in preprocessor is inadequate.  The convention will be documented
in the {\it Developer's Guide} no later than Milestone G (April 2004).

\item[Macros] Standard macros will be identified in later versions
of this Guide.  A full set of standard macros will be identified no later
than Milestone G (April 2004).  
...
Variables are generally assumed to be of default \texttt{KIND}. There
may sometimes be reason to specify the \texttt{KIND} of a variable:

\begin{enumerate}
\item If \texttt{KIND} must be specified for reasons of precision, the
  \texttt{f90} intrinsics \texttt{SELECTED\_REAL\_KIND} and
  \texttt{SELECTED\_INT\_KIND} must be used.
\item If \texttt{KIND} must be specified in order to control the
  storage size (bytelength) of a variable (typically in communication
  and I/O code) it must be done using the parameters \texttt{r8\_kind},
  \texttt{r4\_kind}, \texttt{i8\_kind}, \texttt{i4\_kind} and
  \texttt{i2\_kind}, supplied by the module \texttt{platform},
  which sets various values that are specific to the computing
  platform. The \texttt{platform} module sets these values to the
  appropriate \texttt{KIND} values for FP and integer variables of the
  required bytelength.
\end{enumerate}
...
\begin{enumerate}
\item Character variables that are \emph{arguments} to routines must
...
\item Procedures that perform the same function on different datatypes
  (e.g of differing type, kind or rank) must have a single generic
...
\item Optional arguments, if any, should \emph{follow} the required
  ...
  argument keywords.
\item Argument lists should be short. If necessary related elements of
...
\item[mkmf] The
  ...
  will be used to generate Makefiles with correct dependencies for F90
...
we believe will lead pleasant encounters with clear, legible and
