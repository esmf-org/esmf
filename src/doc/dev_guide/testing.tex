...
The regression test results web page: \htmladdnormallink{{\it here.}}{http://www.earthsystemmodeling.org/developers/test/daily/2week_results/trunk_results_noheader/platform_sort.shtml} provides a color-coded view of the state of the software.
When a developer finds that a test fails on a particular platform with a particular compiler, sometime the bug is readily identifiedand fixed.
However other times the developer might want to know if the test fails on other platforms and if the failure is related to a compiler, mpi configuration or optimized/debug execution.
...
\begin{itemize}
\item Test:ESMF\_FieldBundleSMMUTest.F90
\item Date:Feb 08
\end{itemize}
