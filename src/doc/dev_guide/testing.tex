%===============================================================================
% CVS $Id: testing.tex,v 1.23 2010/04/19 17:29:06 garyblock Exp $
...
\paragraph{Specifying Test Harness tests}
...
files which are located within the \texttt{tests/harness\_config} directory of each 
supported class; currently only ESMF\_ARRAY and ESMF\_FIELD. These configuration files are 
...
tests and as a separate stand-alone test.

\paragraph{Running Test Harness Tests as Part of Unit Tests}
When running as an integral part of the unit tests, the makefile contained in the
\testtt{tests} directory of each supported class is executed through the run_unit_tests target.
...
\begin{itemize}
\item RUN_ESMF_TestHarnessField_default - Used to verify functionality of the test harness
\item RUN_ESMF_TestHarnessField_1 - Basic test of 2D and 3D regrid
\end{itemize}
...
\paragraph{Invoking a Single Test Harness Test Case}
...
\testtt{NP} with the \testtt{run_test_harness} target.

For example,
\begin{verbatim}
gmake TESTHARNESSCASE=field_1 NP=4 run_test_harness
\end{verbatim}
will run the test haress test case, \testtt{field_1} on 4 processors.

Each test case is defeined by a series of configuration files in the
\texttt{<classdir>/tests/harness_config} directory.  
All of the configuration files for a particular test case will be prefixed with 
\texttt{<casename>\_}.  The top level configuration file will have a suffix of \texttt{\_test.rc}.
Thus, the top level configuration file for the \texttt{field_1} test case will be
\testtt{field_1_test.rc}.

The next section describes the format of the top level resource file.

\paragraph{Top Level Configuration File}
The top level configuration file is called \texttt{<casename>\_test.rc} and is located 
in the \texttt{tests/harness\_config} directory of each class; e.g. 
\texttt{esmf/src/Infrastructure/\$CLASS/tests/test\_harness}. 
