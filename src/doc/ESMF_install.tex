% $Id: ESMF_install.tex,v 1.20 2003/04/28 18:24:10 cdeluca Exp $

A user has two options for working with the ESMF.  The first option 
is to download the full ESMF source code, and compile and link
...
{ESMF\_COUPLED\_WAVE} executable.  In this case the user will need
to follow all the instructions in this section.

The second option is to download a pre-built ESMF shared object 
file for a particular platform.  The user who chooses this
approach can then skip ahead to Section~\ref{UsingLibrary}.

\subsection{Downloading the ESMF Software}

Major releases of the ESMF software can be downloaded from 
the {\bf User Links and Downloads} page on the ESMF 
