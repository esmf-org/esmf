% $Id: ESMF_install.tex,v 1.92 2010/09/27 06:00:05 eschwab Exp $
...
\label{sec:lapack}
The \htmladdnormallink{LAPACK}{http://www.netlib.org/lapack} library is 
optionally used in the Mesh class to compute Patch Recovery regridding weights. 
This regridding method requires solving local least squares problems, and 
uses the LAPACK {\it DGELSY} solver.
...
\item[ESMF\_LAPACK] Possible value: {\it not set}, {\tt "OFF"}, {\tt "mkl"}, 
{\tt "netlib"}, {\tt "scsl"}, {\it <userstring>} (default depends on system).

\begin{description}
\item[{\it not set}] ESMF will be compiled with LAPACK-dependent features
on systems where the library is located in a vendor-defined location.  On
other systems, the default is OFF.

\item[{\tt "OFF"}] (default on most system): Disables LAPACK-dependent code, regardless of system.
 
\item[{\tt "mkl"}] Use the Intel MKL library.  Sets {\tt ESMF\_LAPACK\_LIBS} to
{\tt "-lmkl\_lapack -lmkl"} unless it is already defined in the user 
environment.

\item[{\tt "netlib"}] (default for PGI): Sets {\tt ESMF\_LAPACK\_LIBS} to 
{\tt "-llapack -lblas"}, unless it is already defined in the user environment.

\item[{\tt "scsl"}] (default for IRIX64): Sets {\tt ESMF\_LAPACK\_LIBS} to 
{\tt "-lscs"}, unless it is already defined in the user environment.

\item[{\it <userstring>}] Enables LAPACK-dependent code, but does not set a 
default for {\tt ESMF\_LAPACK\_LIBS}.  {\tt ESMF\_LAPACK\_LIBS}, and if 
required, {\tt ESMF\_LAPACK\_LIBPATH}, must be explicitly set in the user 
environment.
\end{description}
...
\htmladdnormallink{NETCDF}{http://www.unidata.ucar.edu/software/netcdf/} form. 
...
{\tt "-lnetcdff -lnetcdf\_c++ -lnetcdf"}.  This option is useful for systems 
which have the Fortran and C bindings archived in seperate library files; 
the C++ bindings are always in a separate library file.  
...
{\tt "-lnetcdf\_c++ -lnetcdf"}.  This option is useful when the Fortran and 
C bindings are archived together in the same library file, alongside the 
always separate C++ bindings library file.  The {\tt ESMF\_NETLIB\_INCLUDE} 
and {\tt ESMF\_NETCDF\_LIBPATH} environment variables will be also used, 
if defined.
...
\item[ESMF\_NETCDF\_LIBS] Typical value: {\tt "-lnetcdf\_c++ -lnetcdf"} 
(no default).
...
{\tt ESMF\_NETCDF\_LIBS} is set to {\tt "-lnetcdf\_c++ -lnetcdf"}.  
When {\tt ESMF\_NETCDF} is set to {\tt "split"}, {\tt ESMF\_NETCDF\_LIBS} 
is set to {\tt "-lnetcdff -lnetcdf\_c++ -lnetcdf}.
\end{description}

\subsubsection{PNETCDF}
\label{sec:pnetcdf}
ESMF provides the ability to write mesh weights in 
\htmladdnormallink{PNETCDF}{http://trac.mcs.anl.gov/projects/parallel-netcdf} 
form.  The following environment variables enable, and specify the name and 
location of the desired PNETCDF library and associated header files:
...
{\tt ESMF\_PNETCDF\_LIBPATH} environment variables will be also used, 
if defined.
...
NetCDF formats through a third-party IO software library, 
\htmladdnormallink{ParallelIO (PIO)}{http://code.google.com/p/parallelio/}. 
The following environment variable enables the functionality of the PIO 
library. The binary format is supported as long as MPI\_IO 
is enabled. To enable NetCDF IO format, the 
{\tt ESMF\_PNETCDF} (see \ref{sec:pnetcdf}) and/or 
{\tt ESMF\_NETCDF} (see \ref{sec:netcdf}) 
...
via the \htmladdnormallink{XERCES}{http://xerces.apache.org/xerces-c/} library.
The following environment variables enable, and specify the name and location
...
{\tt ESMF\_XERCES\_LIBPATH} environment variables will be also used, 
if defined.
