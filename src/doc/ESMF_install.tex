% $Id: ESMF_install.tex,v 1.65 2008/09/15 23:48:02 w6ws Exp $
...
Some pieces of the ESMF library can offer enhanced capabilities when
...
\item[ESMF\_LAPACK]  Possible value: ON (default depends on system)

When defined, enables the use of LAPACK.

\item[ESMF\_LAPACK\_LIBPATH] Typical value:/usr/local/lib (default depends on
system)

Specifies the path where the LAPACK library is located.

\item[ESMF\_LAPACK\_LIBS] Typical value: "-l lapack -l blas" (default depends
on system)

Specifies the linker directives needed to link the LAPACK library to the
application.  Note that on some system, the BLAS library must also be
named.
\end{description}
...
\item[ESMF\_NETCDF] Possible value: ON (default none)

When defined, enables the use of NETCDF.

\item[ESMF\_NETCDF\_INCLUDE] Typical value: /usr/local/include (default none)

Specifies the path where the NETCDF header files are located.

\item[ESMF\_NETCDF\_LIBPATH] Typical value: /usr/local/lib (default none)

Specifies the path where the NETCDF library is located.

\item[ESMF\_NETCDF\_LIBS] Typical value: -lnetcdf (default none)
Specifies the linker directives needed to link the NETCDF library to the
application.  For example, ESMF\_NETCDF\_LIBS=-lnetcdf.  On some systems,
seperate libraries are used for Fortran and C bindings.  In this case,
one can set the environment variable, for example, as
ESMF\_NETCDF\_LIBS="-lnetcdff -lnetcdf".
\end{description}
...
\item[ESMF\_PNETCDF] Possible value: ON (default none)

When defined, enables the use of PNETCDF.

\item[ESMF\_PNETCDF\_INCLUDE] Typical value: /usr/local/include (default none)

Specifies the path where the PNETCDF header files are located.

\item[ESMF\_PNETCDF\_LIBPATH] Typical value: /usr/local/lib (default none)

Specifies the path where the PNETCDF library is located.

\item[ESMF\_PNETCDF\_LIBS] Typical value: -lpnetcdf (default none)
