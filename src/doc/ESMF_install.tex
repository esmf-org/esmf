% $Id: ESMF_install.tex,v 1.89 2010/09/16 18:53:00 samsoncheung Exp $
...
NetCDF formats through a third-party IO software, ParallelIO (PIO).
The following environment variables enable, and specify the name and 
location of the desired PIO library and associated header files:
...
an external PIO library that built separately the user.  Thus, the library and 
...
If {\tt ESMF\_PIO} is enabled, it supports the IO format of binary 
and NetCDF files.  To enable the feature of NetCDF IO format, the
