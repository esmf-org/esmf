...
\smallskip
 ...
 \begin{itemize}
 \item Structured Grids ({\tt ESMF\_Grid}) composed of a single logically rectangular patch
 \item Unstructured Meshes ({\tt ESMF\_Mesh}) composed of hexahedrons (e.g. cubes) and tetrahedrals
 \item A set of disconnected points ({\tt ESMF\_LocStream}) 
 \end{itemize}
...
\subsubsection{Interpolation methods: first-order conservative}\label{sec:interpolation:conserve}
 ...
 exact opposite side of the Earth (antipodal points). 
 However, the user can work around both of these problems by inserting an extra node to break up the long edge into two smaller ones, or by breaking the large target grid cells 
