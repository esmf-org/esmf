% $Id: ESMF_infradataoverview.tex,v 1.32 2012/10/19 23:16:39 oehmke Exp $
...
\subsection{Regrid}\label{sec:regrid}
 ...
 To see what types of grids and other options are supported in the two types of regridding and their testing status, please see the \htmladdnormallink{ESMF Regridding Status}{http://earthsystemmodeling.org/esmf_releases/non_public/ESMF_5_3_0/esmf_5_3_0_regridding_status.html} webpage for this version of ESMF.
 Figure~\ref{Regrid Options} shows a comparison of different regrid methods and where they can be found in the documentation. 

 The rest of this section further describes various options available in ESMF regridding. It starts with a description of the three interpolation methods available in regridding.
...
Name & Access via & Inputs & \multicolumn{2}{|c|}{Outputs} & Description\\ 
     &            &        &  Weights & RouteHandle        &            \\ 
\hline
ESMF\_FieldRegridStore() & Subroutine call & Field object & yes  & yes & Section~\ref{api:esmf_fieldregridstorenx} \\
\hline
ESMF\_FieldBundleRegridStore() & Subroutine call & Fieldbundle object & no  & yes & Section~\ref{api:esmf_fieldbundleregridstore} \\
\hline
ESMF\_RegridWeightGen() & Subroutine call & Grid files & yes  & no & Section~\ref{api:esmf_regridweightgen} \\
\hline
ESMF\_RegridWeightGen & Application & Grid files & yes  & no & Section~\ref{sec:ESMF_RegridWeightGen} \\
\hline
\end{tabular}
\label{Regrid Options}
\caption{Regrid Options}
\end{table}
