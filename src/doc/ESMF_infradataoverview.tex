...
\subsection{Regrid}\label{sec:regrid}
 ...
 To see what types of grids and other options are supported in the two types of regridding and their testing status, please see the \htmladdnormallink{ESMF Regridding Status}{http://earthsystemmodeling.org/esmf_releases/non_public/ESMF_6_2_0/esmf_6_2_0_regridding_status.html} webpage for this version of ESMF.
...
destination grid type, and interpolation method. To avoid the combinatorial growth of the number of pairs
...
internal format. The internal grid format used by the ESMF regrid weight calculation is a finite element
unstructured mesh. This was chosen because it was the most general format and all the others could be 
converted to it. The finite element unstructured mesh is based on the SIERRA~\cite{Sierra} package 
developed at Sandia National Laboratory. It also contains a version of the Zoltan~\cite{Zoltan} library 
for calculating mesh partitions. The internal finite element mesh contains a range of functionality that 
ESMF uses for weight calculation. It can create a mesh of triangles, quadrilaterals, hexahedrons and 
...
vertices and other parts of the mesh. It also contains a range of other useful functionality,
for example the capability to redistribute a mesh to a new distribution, halo communication, etc. 
