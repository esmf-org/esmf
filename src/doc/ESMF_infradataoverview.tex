% $Id: ESMF_infradataoverview.tex,v 1.14 2004/04/20 21:26:33 cdeluca Exp $
...
int/I4, double/R8, and logical ({\tt ESMF\_Logical}) types, and 
char */character strings.  (Arrays of character strings are not 
supported.)

Attributes can be retrieved by name.  A count of all Attributes 
associated with a data object can also be returned, and
then Attributes can also be queried and retrieved by number, 
for iterating over an unknown Attribute list.  The arguments to 
the {\tt <Class>\_GetAttribute} routines are typed and overloaded, 
so the type of the Attribute must match the type of the argument 
receiving it.

Currently Attributes can be added to Fields and Bundles.
