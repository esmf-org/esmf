...
! Create grid and set grid attributes
    ...
    call ESMF_CFIOGridSet(grid, im=72, jm=46, lon=lon, lat=lat, lev=lev, levUnit='layer')
    call ESMF_CFIOGridSet(grid, ak=ak, bk=bk)
    call ESMF_CFIOGridSet(grid,standardName='atmosphere_hybrid_sigma_pressure_coordinate')
    call ESMF_CFIOGridSet(grid, coordinate = 'eta')
    call ESMF_CFIOGridSet(grid,formulaTerm='a: ak b: bk ps: ps')
...
! Create CFIO object and set global attributes
    ...
    call ESMF_CFIOSet(cfio, attintName='nstep', attInt=ak1 )
    call ESMF_CFIOSet(cfio, attIntName='ptop', attInt=bk1 )
    call ESMF_CFIOSet(cfio, attCharName='att1', attChar='test') 
    call ESMF_CFIOSet(cfio, attCharName='att2', attChar='another test') 
...
!    call ESMF_CFIOFileCreate(cfio)
    call ESMF_CFIOFileCreate(cfio, format = 'GrADS')
...
! Close the file
    ...
    if ( passed ) then
       print *, "testw_ex3: Passed"
    else
       print *, "testw_ex3: NOT Passed"
    end if
