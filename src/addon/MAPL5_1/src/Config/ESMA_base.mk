...
ifndef ESMFMKFILE
  $(error When compiling MAPL within ESMF distribution, ESMFMKFILE must be defined)
else
  $(info Using ESMF makefile fragment: $(ESMFMKFILE))
  include $(ESMFMKFILE)
  ESMFMKFILE_contents := $(shell cat $(ESMFMKFILE))
  ifneq "$(findstring ESMF_COMPILER: gfortran, $(ESMFMKFILE_contents))" ""
    ESMA_FC = gfortran
  endif
  ifneq "$(findstring ESMF_COMPILER: intel, $(ESMFMKFILE_contents))" ""
    ESMA_FC = ifort
  endif
  ESMADIR = $(ESMF_INTERNAL_DIR)/src/addon/MAPL5_1
endif
...
# If defined, append flags from ESMFMKFILE
ifdef ESMFMKFILE
  fFLAGS   += $(ESMF_F90COMPILEPATHS)
  f90FLAGS += $(ESMF_F90COMPILEPATHS)
  FFLAGS   += $(ESMF_F90COMPILEPATHS)
  F90FLAGS += $(ESMF_F90COMPILEPATHS)
  FPPFLAGS += $(ESMF_F90COMPILEPATHS)
  LDPATH   += $(ESMF_F90LINKPATHS) $(ESMF_F90ESMFLINKLIBS)
endif
