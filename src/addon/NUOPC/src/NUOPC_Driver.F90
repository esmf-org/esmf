            ...
            if (NUOPC_GridCompAreServicesSet(is%wrap%modelComp(i))) then
              ...
              do k=1, modelPhaseMap(i)%phaseCount
                if (trim(modelPhaseMap(i)%phaseKey(k)) == trim(phaseString)) &
                  phase = modelPhaseMap(i)%phaseValue(k)
              enddo
              if (phase == 0) cycle ! skip to next i since this is not IPDv02p5
              write (pString, *) phase
              ...
              if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
                line=__LINE__, file=trim(name)//":"//FILENAME)) &
                return  ! bail out
              if (trim(valueString)=="true") cycle ! skip to next i
              ...
              if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
                line=__LINE__, file=trim(name)//":"//FILENAME)) &
                return  ! bail out
              if (trim(valueString)=="true") someProgress=.true. ! toggle flag
