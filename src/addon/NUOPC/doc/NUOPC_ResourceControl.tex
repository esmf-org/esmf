...
\subsubsection{Configuring Threaded Child Components}
...
\subsubsection{Driver Queries on Threaded Child Components}

There are cases where the driver component needs to know whether a child component is actively using a specific PET given to it, or whether that PET is paused during child execution --- contributing its PEs to support child-level multithreading instead. To access this information, the driver must first query the child component for its VM:

\begin{verbatim}
type(ESMF_GridComp)     :: child
type(ESMF_VM)           :: vm
...
call NUOPC_DriverGetComp(driver, compLabel="MODEL1", comp=child, rc=rc)
! check rc
call ESMF_GridCompGet(child, vm=vm, rc=rc)
! check rc
\end{verbatim}

With the child VM retrieved, the driver can query it for the {\tt localPet} information:
...
Because this information is queried directly from the child VM, the returned {\tt localPet} reflects the child component's perspective:

\begin{itemize}
\item Active PET: {\tt localPet >= 0}, representing the PET's local index within the child VM.

\item Inactive/Paused PET: {\tt localPet == -1}, indicating that the PET is not active under the child VM.
\end{itemize}

N.B. The MPI communicator queried form the child VM returns {\tt MPI\_COMM\_NULL} on PETs that are inactive under the child component.

\subsubsection{Performance}

For best performance when using NUOPC {\em aware} resource control for hybrid parallelism, it is {\em strongly recommended} to set {\tt OMP\_WAIT\_POLICY=PASSIVE} in the environment. This is one of the standard OpenMP environment variables. The {\tt PASSIVE} setting ensures that OpenMP threads relinquish the hardware threads (i.e. cores) as soon as they have completed their work. Without that setting, ESMF resource control threads can be delayed and context switching between components potentially becomes more expensive.
