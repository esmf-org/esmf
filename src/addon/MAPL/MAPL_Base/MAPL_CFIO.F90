...
!  
! +-======-+ 
!  $Id: MAPL_CFIO.F90,v 1.131.12.4.2.2.4.2 2013-06-14 17:26:40 bmauer Exp $
...
!
  subroutine MAPL_CFIOCreateFromBundle ( MCFIO, NAME, CLOCK, BUNDLE, OFFSET,      &
                                         RESOLUTION, SUBSET, FREQUENCY, LEVELS, DESCR,    &
...
!--------------------
    ...
    VERTGRID: if(HAVE3D) THEN
       ...
       mCFIO%levs = lev
       if (HAVE_ungrd) then
          call ESMF_CFIOGridSet(cfiogrid, levUnit='N/A', RC=STATUS)
          VERIFY_(STATUS)
          call ESMF_CFIOGridSet(cfiogrid, standardName ='N/A', RC=STATUS)
          VERIFY_(STATUS)
...
!
  subroutine MAPL_CFIOCreateFromState ( MCFIO, NAME, CLOCK, STATE, OFFSET,  &
                                        RESOLUTION, SUBSET, LEVELS, DESCR, BUNDLE, &
                                        XYOFFSET, VCOORD, VUNIT, VSCALE,   &
                                        SOURCE, INSTITUTION, COMMENT, CONTACT, &
                                        FORMAT, EXPID, DEFLATE, GC,  ORDER, &
                                          NumCores, nbits,           RC )
...
!-----------------------------------------------------------
    ...
    VARIABLES: do L=1,size(MCFIO%VarDims)
       ...
       FixPole = .false.
       IF ((MCFIO%VarType(L) == MAPL_VectorField) .and. (JM0 == 6*IM0) .and. (Mcfio%JM /= 6*mcfio%IM) .and. (SubSet == .false.) ) FixPole = .true.
...
!------------------------------------------------------
    ...
    if(NUMVARS==0) then
       ...
       do L=1,NUMVARS
          ...
          call ESMF_CFIOVarInfoGet(VARS(L),vname=CFIOVARNAME, vtitle=LONG_NAME, vunits=UNITS, twoDimVar=twoD, RC=STATUS)   
          VERIFY_(STATUS)
          ...
          if (IamRoot) then
             ...
             if ( do_xshift ) then
                 if ( VERB ) print *, Iam // &
                      ': shifting input longitudes by 180 degrees'
                 do k = 1, LM
                    call shift180Lon2D_ ( Gptr3file(:,:,k), im, jm )
                 end do
             end if
             if (change_resolution) then
!                if (IM0 <  IM .or. JM0 < JM) then
...
!                end if ! coarsening
                ...
                Gptr3bundle(1,1,:) = Gptr3file(ISTAR,JSTAR,:)
             else
