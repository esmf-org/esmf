...
!   define ESMF_CFIO,  ESMF_CFIOVarInfo, and ESMF_CFIOGrid objects
    ...
    real :: lon1(72), lat1(46)
    real :: lon2(72), lat2(46)
    ...
    do i =1, im
      lon(i) = -180 + (i-1)*dlon
      lon1(i) = -180 + (i-1)*dlon+0.5*dlon
      lon2(i) = -180 + (i-1)*dlon-0.5*dlon
    end do
    do j =1, jm
      lat(j) = -90 + (j-1)*dlat
      lat1(j) = -90 + (j-1)*dlat+0.5*dlat
      lat2(j) = -90 + (j-1)*dlat-0.5*dlat
    end do
...
! Create grid and set grid attributes
    ...
    call ESMF_CFIOGridSet(grids(1), ak=ak, bk=bk, ptop=0.1)
    ...
    call ESMF_CFIOGridSet(grids(1),formulaTerm='a: ak0 b: bk0 ps: ps p0: ptop0')

    grids(2) = ESMF_CFIOGridCreate(gName='test3d1')
    call ESMF_CFIOGridSet(grids(2), im=72, jm=46, lon=lon1, lat=lat1, lev=lev, levUnit='layer')
    call ESMF_CFIOGridSet(grids(2), ak=ak, bk=bk, ptop=0.2)
    ...
    call ESMF_CFIOGridSet(grids(2),formulaTerm='a: ak1 b: bk1 ps: ps p0: ptop0')

    grids(3) = ESMF_CFIOGridCreate(gName='test3d2')
    call ESMF_CFIOGridSet(grids(3), im=72, jm=46, lon=lon2, lat=lat2, lev=lev, levUnit='layer')
    call ESMF_CFIOGridSet(grids(3), ak=ak, bk=bk, ptop=0.3)
    ...
    call ESMF_CFIOGridSet(grids(3),formulaTerm='a: ak2 b: bk2 ps: ps p0: ptop0')
...
! Close the file
    ...
    if ( passed ) then
       print *, "testw_ex4: Passed"
    else
       print *, "testw_ex4: NOT Passed"
    end if
    stop
   end 
