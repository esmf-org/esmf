...
!		  - replaced 1.3 with 1.1.2.11, including, undoing a
!		    unnecessary fix for parameter NBYTE_PER_WORD;
!		    changes in table contents and formats related
!		    problems; fixes more generic assumptions on long
!		    timer names.
...
!_______________________________________________________________________
  character(len=*),parameter :: myname='m_mall'
	...
	! In this module, NBYTE_PER_WORD is just a unit of measurement
	! for character variables.  It is not the system dependent word
	! size.  No system dependent #ifdef is needed for it.

  integer,parameter :: NBYTE_PER_WORD = 4

  integer,parameter :: NSZ= 64
...
!_______________________________________________________________________
  ...
  if(nword < 0) then
	...
	': invalid argument ("',thread,'"), nword = ',nword
    call ddie(myname_)
  endif
...
!_______________________________________________________________________
  ...
  do i=1,min(n_,MXL)
    ...
    if(ln>lnmax) then
      write(lu,'(a,a)') name(1:ln),' &'
      name=' '
      write(lu,'(a,2(2x,i6),a,2(2x,a6),a)') name(1:lnmax),	&
...
!----------------------------------------
	! Two names are considered the same if their leading NSZ
	! characters are the same in uppercases.

ustring=uppercase(thread)

ith=0
found=.false.
do while(.not.found .and. ith < min(n_,MXL))
  ith=ith+1
  found= ustring == uppercase(name_(ith))
end do
