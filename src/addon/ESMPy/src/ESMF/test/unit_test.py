...
def mesh_test():
    ...
    xcoords2 = np.array([nodeCoord[2*i] for i in range(mesh.size_local[node])])
    ycoords2 = np.array([nodeCoord[2*i+1] for i in range(mesh.size_local[node])])

    status = all(xcoords == xcoords2) and all(ycoords == ycoords2)
