...
\end{verbatim}
will run the test haress test case, \texttt{field\_1} on 4 processors.
...
\texttt{<casename>\_} where <classname> is a unique name for teh test. 
...
As mentioned above, the top level configuration file will be located 
\texttt{<classdir>/tests/harness\_config} directory in the named \texttt{\_test.rc}.
e.g. \texttt{esmf/src/Infrastructure/Field/tests/test\_harness/field_1_test.rc}.
...
problem descriptor files pertaining to a nonexhaustive test configuration. 
...
All of the problem descriptor files pertaining to a particular class live in the 
local directory \texttt{tests/test\_config}.
