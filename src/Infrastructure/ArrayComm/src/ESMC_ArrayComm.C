...
//-----------------------------------------------------------------------------
 ...
 static const char *const version = 
            "$Id: ESMC_ArrayComm.C,v 1.10 2004/04/19 21:54:41 nscollins Exp $";
//-----------------------------------------------------------------------------
...
//
// !ARGUMENTS:
      ESMC_newDELayout *delayout,   // in  - layout 
      int decompids[],           // in  - decomposition identifier for each
                                 //       axis for the Array
      int sizeDecomp,           // in  - size of decomp array
      int localDimCounts[],     // in  - length of each axis for each DE
      int globalDimCounts[],   // in  - array of global dimensions
      int localMaxDimCount[],     // in  - array of maximum counts on any DE per dim
...
// !REQUIREMENTS:  XXXn.n, YYYn.n
    ...
    // get layout size
    int nx, ny, ncount[2];
    delayout->ESMC_newDELayoutGet(NULL, NULL, NULL, NULL, 
                                  0, NULL, NULL, NULL, ncount, 2);
    nx = ncount[0];
    ny = ncount[1];
    int nde = nx*ny;
    int rankx, ranky;
    int bytesperitem = ESMC_DataKindSize(this->kind);
...
//  from current Array
    ...
    int lsize=1;
    for (i=0; i<rank; i++) {
      gsize = gsize * globalDimCounts[i];
      lsize = lsize * (ai_comp[i].max - ai_comp[i].min+1);
      counts[i] = globalDimCounts[i];
    }
...
//jw   delayout->ESMC_newDELayoutGatherArray(this->base_addr, globalDimCounts, 
//jw                                         decompids, sizeDecomp, 
//jw                                         localDimCounts, localMaxDimCount,
//jw                                         ai_comp, ai_total, 
//jw                                         this->kind, gathered->base_addr);
  ...
  switch (sizeDecomp) {
    case 1:
      cout << "no code to handle array rank " << sizeDecomp << " yet\n";
    break;
    case 2:
      {
        ...
        int rbreak[1];
        int rbcount = 0;
        for (i=0; i<sizeDecomp; i++) {
          rmax[i] = globalDimCounts[i];
          rsize[i] = ai_comp[i].max - ai_comp[i].min + 1;
          rsize_tot[i] = ai_total[i].max - ai_total[i].min + 1;
          if (decompids[i] == 0) {
            rbreak[rbcount]=i;
            rbcount++;
          }
        ...
        for (int j=0; j<jMax; j++) {
          j_tot = j + ai_comp[ranky].min;
          sendbuf = this->base_addr;  // void * at this point, cast to char * to increment
          sendbuf = (void *)((char *)sendbuf + 
                        (j_tot*rsize_tot[rankx] + ai_comp[rankx].min) * bytesperitem);
  
          sendcount = rsize[rankx];
          if (j >= rsize[rbreak[0]]) {
            sendcount = 0;
          }
          recvbuf = gathered->base_addr;   // ditto comment above
    ...
    case 3:
      {
        ...
        for (i=0; i<iMax; i++) {
          for (int j=0; j<jMax; j++) {
            sendbuf = this->base_addr;  // void * must be cast to char * to increment
            sendbuf = (void *)((char *)sendbuf + 
    ...
    //gathered->ESMC_ArrayPrint();   // debug only
