% $Id: Grid_desc.tex,v 1.14 2007/05/22 17:22:17 cdeluca Exp $
%
...
each dimension whether it is periodoc, is not periodic, is a pole, or
is a bipole.

ESMF also provides simple standard shapes, {\bf Sphere} and {\bf Box}.
A Sphere is periodic in the longitudinal dimension and has defined
poles.  A Box is a cartesion space up to seven dimensions.  The
periodicity of a Box can be defined on a per-dimension basis by the user.  
...
distributed dimension.  A regular distribution is one in which
...
If the user's grid distribution is too complex for an ESMF
shortcut routine, a DistGrid can be created to specify the grid
distribution in detail.  This DistGrid is then passed into a
Grid create call.
