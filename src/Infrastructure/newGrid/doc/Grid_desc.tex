% $Id: Grid_desc.tex,v 1.26 2007/06/08 13:01:27 cdeluca Exp $
%
...
\caption{Examples of block and regular decompositions for elements
of a grid {\bf a} that is 6x6, and an arbitrary decomposition for 
...
ESMF Grid objects must represent a wide range of grid types, options,
and use cases, some of them quite complex.  As a result, multiple ways to 
build Grid objects are required.  This section describes the two
main stages in Grid creation, and the major alternatives with 
respect to calling sequences.

\subsubsubsection{Stages in Grid Creation}
...
\subsubsubsection{Options for Grid Creation}
...
{\bf or} \newline
ESMF\_GridCreateEmpty()\newline
ESMF\_GridSet(add x dim)\newline
ESMF\_GridSet(add y dim)\newline
...\newline
...
\hline
ESMF\_GridCreate(distGrid)\newline
ESMF\_GridAddCoord(coordArray)\newline
ESMF\_GridCommit(\newline
{\bf or} \newline 
ESMF\_GridCreate(distGrid)\newline
ESMF\_GridAddCoord(<no coordinate values>)\newline
