! $Id: ESMF_RegridConserv.F90,v 1.70 2007/06/23 04:00:39 cdeluca Exp $
!
...
!     igrids and datamaps), this routine constructs a new {\tt Regrid} object
...
!     Conservative Remapping Schemes for IGrids in Spherical Coordinates,
...
!     \item[srcIGrid]
!          {\tt ESMF\_IGrid} corresponding to [srcArray].
!     \item[srcDataMap]
...
!          stored.
!     \item[dstIGrid]
!          {\tt ESMF\_IGrid} corresponding to [dstArray].
!     \item[dstDataMap]
...
! !REQUIREMENTS:  TODO
      ...
      real(ESMF_KIND_R8), dimension(:), allocatable :: &
           ...
           srcUsrArea,            &! src cell area supplied by user through igrid
      ...
      real(ESMF_KIND_R8), dimension(:,:), allocatable :: &
           ...
           dstUsrArea,            &! dst cell area supplied by user through igrid
...
!                               ESMF_CONTEXT, rc)) return
      ...
      if (hasDstData) nC = size(dstLocalCornerX,1)
                                   ...
                                   !      igrid is OK for now, but should be a
                                   !      call to the source igrid eventually
      if (hasDstData) then
      ...
      if (hasDstData) then
        ...
        ! set up user masks for search    ! TODO: combine user and igrid masks
  ...
  !        srcUserMask = srcMask
        ...
        ! allocate various local dst igrid arrays
   ...
   !     srcUsrArea  (:) = ?
        ...
        call ESMF_IGridGet(srcIGrid, horzCoordSystem=coordSystem, rc=localrc)
 !       numSrcCorners = size(srcLocalCornerX,1)   ! TODO: should be from a IGrid call
        numSrcCorners = size(dstLocalCornerX,1)   ! TODO: should be from a IGrid call
...
!          {\tt Regrid}name.
!     \item[srcCornerX]
!          X cell corner coords for gathered source igrid cells that potentially
!          overlap destination domain.
!     \item[srcCornerY]
!          Y cell corner coords for gathered source igrid cells that potentially
!          overlap destination domain.
!     \item[dstCornerX]
!          X cell corner coords for gathered destination igrid cells.
!     \item[dstCornerY]
!          Y cell corner coords for gathered destination igrid cells.
!     \item[srcMask]
...
!EOPI
      ...
      integer ::           &
         ...
         ibDst, ieDst,     &! beg, end of exclusive domain in i-dir of dest igrid
         jbDst, jeDst,     &! beg, end of exclusive domain in j-dir of dest igrid
         ibSrc, ieSrc       ! beg, end of exclusive domain in i-dir of source igrid

      integer :: srcAdd,    &! address in gathered source igrid
                 dstAdd(2)   ! address in dest igrid
      ...
      jDst = 0
      nDst = 0
      ! loop through each cell on source igrid and
      ...
      srcLoop: do srcAdd = ibSrc,ieSrc
        ...
        srcCornerLoop: do corner = 1,numSrcCorners
          ...
          do while (ybeg.ne.yend .OR. xbeg.ne.xend)
            ...
            ! find next intersection of this segment with a igrid
            ! line on the destination igrid.  Offset from last
            ! intersection to nudge into next igrid cell.
            if (coordSystem .eq. ESMF_COORD_SYSTEM_SPHERICAL) then
      ...
      ! now integrate around each cell on destination igrid
        ...
        dstLoopInner: do iDst = ibDst,ieDst
          ...
          dstCornerLoop: do corner = 1,numDstCorners
            ...
            do while (ybeg.ne.yend .OR. xbeg.ne.xend)
              ...
              ! find next intersection of this segment with a igrid line on the
              ! destination igrid.  Offset from last intersection to nudge into
              ! next igrid cell.
              if (coordSystem .eq. ESMF_COORD_SYSTEM_SPHERICAL) then
                call ESMF_RegridConservXSphr(searchAdd, startCorner, &
                                             xIntersect, yIntersect, &
      ...
      if (ESMF_LogMsgFoundAllocError(localrc, "deallocate corner arrays", &
                                     ESMF_CONTEXT, rc)) return
      !
      ! For igrids in spherical coordinates:
      ! correct for situations where N/S pole not explicitly included in
      ! igrid (i.e. as a igrid corner point). if pole is missing from only
      ! one igrid, need to correct only the area and centroid of that
      ! igrid.  if missing from both, do complete weight calculation.
      if (coordSystem .eq. ESMF_COORD_SYSTEM_SPHERICAL) then
...
!EOPI
      ...
      do srcAdd = 1,srcSize
        if (srcArea(srcAdd).lt.-0.01d0) then
          write(logMsg, *) "Source igrid area < -0.01"
      ...
      do jDst   = 1,dstSizeY
        do iDst = 1,dstSizeX
          if (dstArea(iDst,jDst).lt.-0.01d0) then
            write(logMsg, *) "Dest igrid area < -0.01"
            call ESMF_LogWrite(logMsg, ESMF_LOG_WARNING)
      ...
      do n = 1,numlinks
        ...
        ! sum the weight for each dest igrid point
...
!     \item[coordSystem]
!        The {\tt ESMF_CoordSystem} for this igrid to determine which
...
!  intersection of that line segment with a igrid line of an input igrid. 
!  The location of the beginning point in the igrid is returned together
!  with the intersection point.  A coincidence flag is returned if the 
!  segment is entirely coincident with a igrid line.  If the segment
!  lies entirely within a igrid cell (no intersection), the endpoints
!  of the segment are returned as the intersection point.  If the
!  beginning point lies outside the igrid, a location of zero is 
!  returned but an intersection may still be returned if the segment
!  enters the igrid from outside the igrid domain.  If the current segment
...
!        in regrid.
!     \item[coordSystem]
!        The {\tt ESMF_CoordSystem} for this igrid to check for special
...
!EOPI
      ...
      integer ::         &
        ...
        nCells,          &! search igrid size
        numCorners        ! number of corners in each search igrid cell

      logical ::    & 
        reverse,    &! segment in opposite direction of full segment
        outside,    &! true if beg point outside igrid
        found        ! true if igrid cell found

      real (ESMF_KIND_R8) ::     &
        xb, yb, xe, ye,         &! local coordinates for segment endpoints
        x1, y1, x2, y2,         &! coordinates for igrid side endpoints
      ...
      ! search for location of the beginning of the segment in input igrid
    5 cellLoop: do n = 1, nCells
     ...
     !   if (xb.gt.maxval(cornerX) .OR. yb.gt.maxval(cornerY)) cycle cellLoop
        ...
        if (corner.gt.numCorners) then
          ...
          ! if the first part of the segment was outside the igrid the found point
          !   corresponds to the first point inside the igrid. invert the segment
          !   to find first intersection with igrid boundary
          if (outside) then
            xe   = xbeg
            ye   = ybeg
            nLoc = 0
      ...
      ! cell not found - assume beg point outside igrid
      ! take baby steps along segment to find a point inside the igrid
      s2 = s2 + s1
   ...
   10 continue
      ...
      if (found) then
        ...
        intrsctLoop: do corner = 1,numCorners
          ...
          !   number so the intersection is off the igrid line).
...
!     \begin{description}
!     \item[nLoc]
!        The n location in the searched igrid containing the beginning
!        endpoint of the line segment.  If point could not be found,
!        returns a zero signifying point outside igrid.
!     \item[xIntersect, yIntersect]
!        Coordinates of the next intersection of the input line segment
...
!EOPI
      ...
      integer :: &
        localrc,            &
        n,                  &! dummies for addresses
        nCells,             &! number of cells in search igrid
        numCorners,         &! number of corners in each cell of search igrid
        corner, nextCorner   ! loop index, next index

      logical :: & 
        reverse,           &! segment opposite direction of full segment
        outside,           &! point outside of search igrid
        found               ! true if igrid cell found
   ...
   ! loop through igrid to find cell containing search point
   ...
   10 continue
      ...
      ! if the first part of the segment was outside the igrid
      ! the found point corresponds to the first point inside the
      ! igrid. invert the segment to find first intersection with
      ! igrid boundary
      if (outside) then
        ...
        intrsctLoop: do corner = 1,numCorners
          ...
          if (abs(determ).gt.1.d-30) then
            ...
            if (s2.ge.0.0d0 .AND. s2.le.1.0d0 .AND. &
              ...
              ! where computing intersections between two igrids
