! $Id: ESMF_VMComponentUTest.F90,v 1.7 2007/06/22 23:21:42 cdeluca Exp $
!
...
module ESMF_VMComponentUTest_gcomp_mod
  ...
  subroutine mygcomp_register_nexh(gcomp, rc)
    ...
    ! threading features.
    call ESMF_InternGridCompSetVMMinThreads(gcomp, rc=rc)
#endif
...
!------------------------------------------------------------------------------
  ...
  type(ESMF_VM):: vm
  type(ESMF_InternGridComp):: gcomp(1000)
...
!------------------------------------------------------------------------------
  ...
  do j=1, 2
    do i=1, 100
      ...
      gcomp(i) = ESMF_InternGridCompCreate(name='My interngridded component', rc=loop_rc)
      if (loop_rc /= ESMF_SUCCESS) goto 10
  
      call ESMF_InternGridCompSetServices(gcomp(i), mygcomp_register_nexh, loop_rc)
      if (loop_rc /= ESMF_SUCCESS) goto 10

    enddo
    do i=1, 100

      call ESMF_InternGridCompDestroy(gcomp(i), rc=loop_rc)
      if (loop_rc /= ESMF_SUCCESS) goto 10
...
10 continue
  ...
  do j=1, 20
    do i=1, ConThreads
      ...
      call ESMF_InternGridCompSetServices(gcomp(i), mygcomp_register_exh, loop_rc)
      if (loop_rc /= ESMF_SUCCESS) goto 20
