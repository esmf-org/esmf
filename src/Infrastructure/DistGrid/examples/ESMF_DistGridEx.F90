! $Id: ESMF_DistGridEx.F90,v 1.7 2006/11/16 05:20:58 cdeluca Exp $
!
...
!BOC
  ...
  call ESMF_DistGridGet(distgrid, dimExtent=dimExtent, rc=rc) ! get dimExtent
  if (rc /= ESMF_SUCCESS) call ESMF_Finalize(terminationflag=ESMF_ABORT)
  do de=1, 6
    do dim=1, 2
      allocate(indexList(dimExtent(dim, de))) ! allocate list to hold indices
      call ESMF_DistGridGet(distgrid, de=de-1, dim=dim, &
        indexList=indexList, rc=rc)
      if (localPet==0) then
        print *, "indexList for de=",de-1,"along dim=",dim," :: ", indexList
      endif
