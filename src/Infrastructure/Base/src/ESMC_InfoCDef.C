...
void ESMC_InfoBaseSyncDo(const std::vector<long int> &base_addresses, const int &rootPet, const long int &vmAddress, int &esmc_rc) {
  esmc_rc = ESMF_FAILURE;
  try {
...
void ESMC_InfoBaseSync(ESMCI::Info *inqstate, int &rootPet, long int &vmAddress, int &esmc_rc) {
  ...
  try {
    ...
    ESMC_InfoBaseSyncDo(base_addresses, rootPet, vmAddress, esmc_rc);
  }
  ESMC_CATCH_ISOC
}
