! $Id: ESMF_GridWrapUTest.F90,v 1.1 2006/03/03 21:52:46 nscollins Exp $
!
...
!==============================================================================
!BOC
! !PROGRAM: ESMF_GridWrapUTest - Tests related to periodic grids
!
! !DESCRIPTION:
!
! The code in this file tests things specific to periodic grids.
!
!-----------------------------------------------------------------------------
...
!------------------------------------------------------------------------------
    ...
    grid = ESMF_GridCreateHorzXYUni((/ 50, 30 /), origin, &
	                            ...
	                            periodic=(/ ESMF_FALSE, ESMF_FALSE /), &
                                    name="atmgrid", rc=rc)
    write(name, *) "ESMF_GridCreate Test"
    write(failMsg, *) "Did not return ESMF_SUCCESS"
    call ESMF_Test((rc.eq.ESMF_SUCCESS), &
                    name, failMsg, result, ESMF_SRCLINE)
    ...
    write(name, *) "ESMF_GridDistribute Test"
    write(failMsg, *) "Did not return ESMF_SUCCESS"
    call ESMF_Test((rc.eq.ESMF_SUCCESS), &
                    name, failMsg, result, ESMF_SRCLINE)
    ...
    !--------------------------------------------------------------------------
    !--------------------------------------------------------------------------
    ! the gridStart lines should all print the same values (and do not).
    ...
    print *, "F/F gridStart=", gridStart
    write(name, *) "ESMF_GridGetDELocalInfo Test"
    write(failMsg, *) "Did not return ESMF_SUCCESS"
    call ESMF_Test((rc.eq.ESMF_SUCCESS), &
                    name, failMsg, result, ESMF_SRCLINE)
...
! allocate the F90 pointer with the calculated bounds
    ...
    end program ESMF_GridWrapUTest
