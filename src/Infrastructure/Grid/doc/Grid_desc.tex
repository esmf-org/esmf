% $Id: Grid_desc.tex,v 1.56 2011/06/13 18:44:19 oehmke Exp $
%
...
\item [ESMF\_GridCreate1PeriDim()] Allows the user to create a Grid with 1 periodic dimension and supports a range of options for what to do at the pole (see ~Section~\ref{sec:opt:poletype}. Some examples of Grids which can be created here are tripole spheres, bipole spheres, cylinders with open poles. 
...
\begin{sloppypar}
The second set of shortcut methods is a set of methods overloaded under the name {\tt ESMF\_GridCreateTile()}. These methods
allow the user to specify the connectivites at the end of each dimension, by using the ESMF\_GridConn flag. The table below shows the ESMF\_GridConn settings used to create 
standard shapes in 2D using the ESMF\_GridCreateTile() call.  Two values
