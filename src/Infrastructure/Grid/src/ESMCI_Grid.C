// $Id: ESMCI_Grid.C,v 1.36.2.2 2008/02/29 23:20:37 theurich Exp $
//
...
static const char *const version = "$Id: ESMCI_Grid.C,v 1.36.2.2 2008/02/29 23:20:37 theurich Exp $";
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
 ...
 // Rename Grid UndistLBound, UndistUBound to distinguish them from the Array versions 
  ...
  ESMCI_ArraySpec *arrayspec= new ESMCI_ArraySpec();     
  ...
  for (coord=0; coord<dimCount; coord++) {
    ...
    arrayspec->ESMCI_ArraySpecSetRank(coordDimCount[coord]);
    arrayspec->ESMCI_ArraySpecSetTypeKind(typekind);
...
#define SERIALIZE_VAR2D(cp,bufptr,loff,varptr,s1,s2,t) \
  ...
  while (!done) {
    ...
    SERIALIZE_VAR2D(cp, buffer,loffset,didIAllocList,staggerLocCount,dimCount, bool);
...
#define DESERIALIZE_VAR2D(bufptr,loff,varptr,s1,s2,t) \
  ...
  DESERIALIZE_VAR2D( buffer,loffset,didIAllocList,staggerLocCount,dimCount, bool);
