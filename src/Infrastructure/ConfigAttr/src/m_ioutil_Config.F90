...
!BOP
!
! !MODULE: m_ioutil_Config - a F90 module for several convenient I/O functions
!
! !DESCRIPTION:
!
...
! !INTERFACE:
    ...
    subroutine opnieee(lu,fname,status,ier,recl)
      use m_stdio_Config ,only : stderr
      implicit none
...
! !INTERFACE:
    ...
    subroutine luflush(unit)
      use m_stdio_Config, only : stdout
      implicit none
