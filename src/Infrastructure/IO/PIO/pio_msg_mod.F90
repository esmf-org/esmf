...
module pio_msg_mod
  ...
  public :: add_to_file_list, lookupfile
  public :: delete_from_file_list, lookupiodesc, add_to_iodesc_list
    ...
    if(iorank==0) then
       do index=1,numcomps
          ...
          if(ios%io_rank==0) then
             call mpi_irecv(msg, 1, mpi_integer, ios%comproot, 1, &
                            ios%union_comm, req(index), ierr)       
          end if
       enddo
    end if
