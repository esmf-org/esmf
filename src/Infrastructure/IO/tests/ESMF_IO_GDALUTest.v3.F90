...
!                                     regDecompPTile=decomptile, &
  ...
  !------------------------------------------------------------------------
  write(name, *) "Creating a Mesh to test .nc I/O"
...
!       elementDistgrid=elemDistgrid, &
  ...
  call ESMF_MeshWrite(mesh,"ncfile_mesh",rc=rc)

  write(name, *) "Destroying .nc mesh..."
...
!>>       status=ESMF_FILESTATUS_UNKNOWN, rc=rc)
  ...
  call ESMF_FieldGet( Gridfield, farrayPtr=gptr, rc=rc)
  call ESMF_FieldGet( Meshfield, farrayPtr=mptr, rc=rc)
  write(*,*) 'Gridfield: ', maxval(gptr), maxval(mptr)
  gptr => null()
