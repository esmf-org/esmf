% $Id: PhysGrid_req.tex,v 1.7 2002/04/23 15:13:48 cnh Exp $
%===============================================================================
...
Mechanisms shall be provided to specify grids numerically i.e.
...
\begin{reqlist}
{\bf Priority:} \\ {\bf Source:} \\ {\bf Status:} \\ {\bf Verification:} \\ 
{\bf Notes: Locations in a mapping are to be given in standard, S.I., units as follows.
For horizontal locations, coordinates of meters (Cartesian domain), degrees latitude
and degrees longitude (speherical polar domain) and meters and degrees (Cylindrical
...
\ssreq{Initialization of grid cell volumes with vertex based numerical values}
...
It should be possible to initialize a physical grid from
a set of numerical values that are calculated by a component application.
\ssreq{Initialization from File}

\sreq{Initialization by a Function}
\ssreq{Spectral Spherical Harmonics Code}
\ssreq{Spectral FFT Code}
\ssreq{Grid Point Cartesian Code}
\ssreq{Grid Point Spherical Polar Code}
\ssreq{Grid Point User Supplied Function}
\ssreq{Spectral User Supplied Function}

\sreq{Modification Tracking}

\req{Physical Grid Attributes and Queries}
\sreq{X-axis index labeling}
\sreq{Y-axis index labeling}
\sreq{R-axis index labeling}
\sreq{Labeling of quantities}
\ssreq{An associative form}
\sreq{Quadrilateral curvilinear othogonal A-grid, physical grid values}
\sreq{Quadrilateral curvilinear othogonal B-grid, physical grid values}
\sreq{Quadrilateral curvilinear othogonal C-grid, physical grid values}
\sreq{Quadrilateral curvilinear othogonal D-grid, physical grid values}
\sreq{Quadrilateral curvilinear othogonal E-grid, physical grid values}
\sreq{Catchment Grid software-convention}
\sreq{Ungridded Grid software-convention}
\sreq{Mapping queries}
\ssreq{Index based}
\ssreq{Lines of constant index}
\sreq{Inverse Mapping queries}
