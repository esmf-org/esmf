%$Id: PhysGrid_req.tex,v 1.19 2002/05/23 20:07:18 jwolfe Exp $
...
\sreq{Physical grids may be internally generated}
...
\sreq{Refinement}
A physical grid may be interpolated to generate a physical grid with an equivalent physical
...
\sreq{1- 2- or 3- dimensional physical grids}
Physical grids may have up to 3 dimensions, but must have at least as many dimensions as the
...
\ssreq{Stand-alone physical grid generation examples}
...
\ssreq{Physical grid topology consistency checking}
...
%cnh m%titude
%cnh ===============================================================================
%cnh \req{Initialization of a Physical Grid}
%cnh \begin{reqlist}
...
%cnh The Physical Grid element design should not preclude the user from using alternative
...
%cnh A method shall be provided to check that physical grid cells do not overlap. 
%cnh \begin{reqlist}
%cnh {\bf Priority:} 2 \\
...
%cnh Upon creation, the name and a pointer to each physical grid shall be stored in a
%cnh registry.  A pointer to any physical grid may be determined given its name.
%cnh \begin{reqlist}
%cnh {\bf Priority:} 3 \\
%cnh {\bf Source:} \\
%cnh {\bf Status:} Proposed \\
...
%cnh A physical grid may be queried for the number of dimensions, which is
...
%cnh A physical grid may be queried for the local or global lengths of each of its dimensions.
%cnh \begin{reqlist}
%cnh {\bf Priority:} 2 \\
%cnh {\bf Source:} \\
%cnh {\bf Status:} Proposed \\
...
%cnh A physical grid may have an arbitrary number of text or numeric attributes,
%cnh which may be added, set and queried.  Each attribute has a text name by which it
%cnh can be queried.  Also, a physical grid can be queried for a list of all global
%cnh attribute names.
%cnh 
%cnh \begin{reqlist}
%cnh {\bf Priority:} 2 \\
...
%cnh A physical grid may have an arbitrary number of masks associated with it.
%cnh \begin{reqlist}
%cnh {\bf Priority:} 2 \\
%cnh {\bf Source:} ? \\
%cnh {\bf Status:} Proposed \\
