...
!-------------------------------------------------------------------------------
  ...
  contains
    ...
    subroutine test_ReverseAlarms(testPass, rc)
      ...
      type(ESMF_Time) :: time
      logical :: esmfalarm_ring, firstalarm_ring
      logical :: allCorrect
      ...
      if(status /= ESMF_SUCCESS) call ESMF_Finalize()
      firstalarm_ring = ESMF_AlarmIsRinging(firstalarm,rc=status)
      call verify_(status)
      if (.not. firstalarm_ring) then
        allCorrect = .false.
      end if
      ...
      do i=1,nstep
        ...
        if (modulo(i,4) == 0) then
          ! Time step of 15 minutes, alarm rings every hour, so alarm should ring every 4th time step
          if (.not. esmfalarm_ring) then
            allCorrect = .false.
          end if
        else
        ...
        if (modulo(i,8) == 0) then
          ! Time step of 15 minutes, alarm rings every two hours, so alarm should ring every 8th time step
          if (.not. firstalarm_ring) then
            allCorrect = .false.
          end if
        else
          if (firstalarm_ring) then
            allCorrect = .false.
          end if
        end if
      enddo
      ...
      do
        ...
        esmfalarm_ring = ESMF_AlarmIsRinging(esmfalarm,rc=status)
        call verify_(status)
        if (modulo((i-3),4) == 0) then
          ! Time step of 15 minutes, with time starting at 45 minutes past the hour, alarm
          ! rings every hour, so alarm should ring every 4th time step starting with i==3
          if (.not. esmfalarm_ring) then
            allCorrect = .false.
          end if
        else
        ...
        firstalarm_ring = ESMF_AlarmIsRinging(firstalarm,rc=status)
        call verify_(status)
        if (modulo((i-7),8) == 0) then
          ! Time step of 15 minutes, with time starting at 45 minutes past an odd-numbered
          ! hour, alarm rings every two hours, so alarm should ring every 8th time step
          ! starting with i==7
          if (.not. firstalarm_ring) then
            allCorrect = .false.
          end if
...
subroutine Test_AlarmAdvRewind(testPass, rc)
  ...
  type(ESMF_Time) :: time
  type(ESMF_Alarm) :: esmfalarm900, esmfalarm1800
  logical :: esmfalarm900_ring, esmfalarm1800_ring
  logical :: allCorrect
  ...
  esmfalarm900 = ESMF_AlarmCreate(clock=clock,ringInterval=tint,ringTime=start_time,sticky=.false.,name='alarm_900',rc=status)
  call verify_(status)
  call ESMF_TimeIntervalSet(tint,s=1800)
  esmfalarm1800 = ESMF_AlarmCreate(clock=clock,ringInterval=tint,ringTime=start_time,sticky=.false.,name='alarm_1800',rc=status)
  call verify_(status)

  esmfalarm900_ring = ESMF_AlarmIsRinging(esmfalarm900, rc=status)
  call verify_(status)
  if (.not. esmfalarm900_ring) then
    allCorrect = .false.
  end if
  esmfalarm1800_ring = ESMF_AlarmIsRinging(esmfalarm1800, rc=status)
  call verify_(status)
  if (.not. esmfalarm1800_ring) then
    allCorrect = .false.
  end if
...
!------------------------------------------------------------------------
  subroutine ForwardAlarm_Test2(rc)
    ! Test 2: c.t0 = a.t0, c.dt = 10 min, a.dt = 20 min
    ...
    do n = 1, 6
      ...
      if (modulo(n,2) == 0) then
        ! Alarm should be ringing on even time steps
        if (.not. ringing) then
          allCorrect = .false.
        end if
      else
        ! Alarm should NOT be ringing on odd time steps
        if (ringing) then
          allCorrect = .false.
        end if
      end if
...
!------------------------------------------------------------------------
  subroutine ForwardAlarm_Test3(rc)
    ! Test 3: c.t0 = a.t0, c.dt = 20 min, a.dt = 10 min
    !
    ! This should raise an error: it's an error to create an alarm with alarm timeInterval
    ! less than clock timeStep
    ...
    if(status /= ESMF_SUCCESS) then
      ! This is the expected result: the status of ESMF_AlarmCreate should be an error, so
      ! in this case, this test returns SUCCESS
      rc = ESMF_SUCCESS
    else
      ! On the other hand, if ESMF_AlarmCreate succeeded, then this test has failed
      rc = ESMF_FAILURE
    end if
...
!------------------------------------------------------------------------
  subroutine ForwardAlarm_Test5(rc)
   ! Test 5: c.t0 = 00:00:00, a.t0=00:04:00, c.dt = a.dt = 10 min, sticky (an alarm won't ring)
   !
   ! This should raise an error: it's an error to create an alarm where the alarm interval
   ! is the same as clock interval but ringTime is misaligned with clock time: in this
   ! case, the alarm will never ring.
...
!------------------------------------------------------------------------
  subroutine ForwardAlarm_Test6(rc)
    ! Test 6: c.t0 = 00:00:00, a.t0 = 00:30:00, c.dt = 10 min, a.dt = unspecified (one shot)
    ...
    do n = 1, 6
      ...
      if(status /= ESMF_SUCCESS) call ESMF_Finalize()
      if (n == 3) then
        ! Alarm should be ringing on the 3rd time step, when the clock time should match
        ! the alarm's initial time
        if (.not. ringing) then
          allCorrect = .false.
        end if
      else
...
!------------------------------------------------------------------------
  subroutine ForwardAlarm_Test7(rc)
    ! Test 7: c.t0, a.t0 unspecified default to c.t0, c.dt = 10 min, a.dt = 10 min
...
!------------------------------------------------------------------------
  subroutine ReverseAlarm_Test1(rc)
    ! Test 1: c.t0 = a.t0, c.dt = a.dt = 10 min, time reversed
...
!------------------------------------------------------------------------
  subroutine ReverseAlarm_Test2(rc)
    ! Test 2: c.t0 = a.t0, c.dt = 10 min, a.dt = 20 min, time reversed
...
!------------------------------------------------------------------------
  subroutine ReverseAlarm_Test3(rc)
    ! Test 3: c.t0 = a.t0, c.dt = 20 min, a.dt = 10 min, time reversed
    !
    ! This should raise an error: it's an error to create an alarm with alarm timeInterval
    ! less than clock timeStep
...
!------------------------------------------------------------------------
  subroutine ReverseAlarm_Test4(rc)
    ! Test 4: c.t0 = a.t0, c.dt = 10 min, a.dt = 12 min, time reversed
...
!------------------------------------------------------------------------
  subroutine ReverseAlarm_Test5(rc)
    ! Test 5: c.t0 = 00:00:00, a.t0=00:04:00, c.dt = a.dt = 10 min, sticky, time reversed (an alarm won't ring)
    !
    ! This should raise an error: it's an error to create an alarm where the alarm interval
    ! is the same as clock interval but ringTime is misaligned with clock time: in this
    ! case, the alarm will never ring.
...
!------------------------------------------------------------------------
  subroutine ReverseAlarm_Test6(rc)
    ! Test 6: c.t0 = 00:00:00, a.t0 = 00:30:00, c.dt = 10 min, a.dt = unspecified (one shot), time reversed
...
!------------------------------------------------------------------------
  ...
  subroutine ForwardReverseAlarm_Test3(rc)
    ! Test 3: c.t0 = a.t0, c.dt = 10 min, a.dt = 12 min
