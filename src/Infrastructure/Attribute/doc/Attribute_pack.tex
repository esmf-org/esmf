% $Id: Attribute_pack.tex,v 1.16 2010/10/07 18:04:40 murphysj Exp $
%
...
\begin{tabular}{|p{8cm}|p{20cm}|p{10cm}}
     ...
     {\tt SimulationDuration} & The length of time a simulation runs.&  N/
...
\begin{tabular}{|p{8cm}|p{20cm}|p{10cm}|}
     ...
     {\tt MachineProcessorType} & The type of computer chip used in a particular computer platform. & Altix, AMD x86-64, Bluegene, G4, G5, Intel em64t, intel IA-64, Itanium, NEC, operteron, Origin3800, Pentium 3, Pentium 4, SP, SPARC, X1, Xeon, XT3-4, ZX6000\\
...
\begin{itemize}
    ...
    \item Usage: The Responsible Party package is unique in that the user should first select the type of Responsible Party they wish to define. This is done via
the ResponsiblePartyRole attribute. Then, depending upon the role chosen, either an OrganizationName or IndividualName along with the corresponding attribute (e.g. FundingSource) is set. In a future release of ESMF, the user will not need to set the OrganizationName or IndividualName, which are used only in the output. This will happen automatically by ESMF.  
\end{itemize}
...
\begin{tabular}{|p{8cm}|p{20cm}|p{10cm}|}
     ...
     {\tt Author} & The abbreviation of an individual or organization associated with a model component or simulation. & N/A \\
     {\tt EmailAddress} & The email address that others can use to ask questions about a model component. & N/A \\
     {\tt FundingSource} & The entity that funded a model component or simulation. & N/A\\
     {\tt IndividualName} & The name with an author, contact, or principal investigator responsible party type. & N/A \\
     {\tt OrganizationName} & The name of an organization associated with a funding source or centre. & N/A \\
     {\tt PhysicalAddress} & The address of the person designated to provide information about a model component. & N/A \\
     {\tt PrincipalInvestigator} & The scientist in charge of an experiment or research project. & N/A \\
...
\begin{tabular}{|p{8cm}|p{20cm}|p{10cm}|}
    ...
    {\tt SpatialRegriddingMethod} & Method used to interpolate a field from one grid (source grid) to another (target grid). & Conservative, Non-conservative, None\\ 
    {\tt SpatialRegriddingType} & Characteristics of the scheme used to interpolate a field from one grid (source grid) to another (target grid).& 2D-FirstOrder, 2D-SecondOrder, 3D-FirstOrder, 3D-SecondOrder\\ 
...
\begin{tabular}{|p{8cm}|p{20cm}|p{10cm}}
     ...
     {\tt Type} & The form of the input condition (e.g. initial condition or boundary condition). &  CIM Inputs \\
     ...
     {\tt SpatialRegriddingType} & Characteristics of the scheme used to interpolate a field from one grid (source grid) to another (target grid).& CIM Inputs\\ 
     ...
     {\tt LongName} & An ad-hoc long descriptive name which may, for example, be used for labeling plots & CF General\\
     {\tt LongTitle} & The text of the citation or pointer (e.g. URL) that further describes a model component or simulation. & CIM Citation\\
     ...
     {\tt PhysicalAddress} & The address of the person designated to provide information about a model component. & CIM Responsible Party\\
     {\tt PhysicalDomain} & A description of the geographic range being simulated. & ESG Basic Component \\
     {\tt PresentationForm} & A description of the type of citation & documentDigital, documentHardcopy, imageDigital, imageHardcopy, mapDigital, mapHardcopy, modelDigital, modelHardcopy, profileDigital, profileHardcopy, tableDigital, tableHardcopy, videoDigital, videoHardcopy & CIM Citation\\
     ...
     {\tt ShortName}  & The short\_name is technically not part of the CF stanard but is commonly the name of the variable on the output file and so is distinct from the long\_name & CF General\\
     {\tt ShortTitle} & An abbreviation for the citation.  This could be the short scientific citation (e.g. Murphy, 2009) or the title of a web page. & CIM Citation \\     
     ...
     {\tt URL} & URL of the object being described. Exists in multple packages & CIM Responsible Party, CIM Main, CIM Citation\\
