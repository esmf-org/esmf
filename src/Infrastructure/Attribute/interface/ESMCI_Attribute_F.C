...
//-----------------------------------------------------------------------------
//BOP
// !IROUTINE:  c_ESMC_AttributeCopyReplace - copy an attribute between objects
//
// !INTERFACE:
      void FTN_X(c_esmc_attributecopyreplace)(
//
#undef  ESMC_METHOD
#define ESMC_METHOD "c_esmc_attributecopyreplace()"
//
// !RETURN VALUE:
//    none.  return code is passed thru the parameter list
// 
...
//EOP
  ...
  if (*attcopyflag == ESMF_COPY_VALUE && *atttreeflag == ESMC_ATTTREE_OFF) {
      status = (**destination).root.AttributeCopyReplace((**source).root);
      ESMC_LogDefault.MsgFoundError(status, ESMCI_ERR_PASSTHRU,
        ESMC_CONTEXT, ESMC_NOT_PRESENT_FILTER(rc));
  } else {
    ESMC_LogDefault.MsgFoundError(ESMC_RC_ARG_BAD,
...
}  // end c_ESMC_AttributeCopyReplace
