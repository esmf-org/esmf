% $Id: Array_design.tex,v 1.1 2003/03/10 21:54:19 cdeluca Exp $
...
the routine is simply passed an existing F90 array.   Many of the
...
compilers and platforms, and then use less efficient or more indirect methods for obtaining this information which will be the default if no compiler-specific method has been written.

The Array type is defined at the base level because it is expected
to be useful in Fields, Grids, generic Get/SetAttribute routines, as
well as any other places where the user may specify data of different
types, shapes, and ranks.
...
maintaining the interface; in Fortran the interface to the user will
be simplified by using interface blocks but the number of internal
