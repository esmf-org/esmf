...
!            write(*,*) "relErr=",relErr,farrayPtr(i1,i2),xfarrayPtr(i1,i2)
 ...
 subroutine test_regridPartialVM(rc)
  ...
  ! Fill so everything in just on the first 3 Pets
  ...
  srcGrid=ESMF_GridCreate1PeriDim(minIndex=(/1,1/), maxIndex=(/src_nx, src_ny/), &
                                  indexflag=ESMF_INDEX_GLOBAL, &
                                  regDecomp=regDecomp, petMap=petMap, &
                                  rc=localrc)
  if (localrc /=ESMF_SUCCESS) then
    rc=ESMF_FAILURE
    return
  ...
  do lDE=0,srclocalDECount-1
     ...
     call ESMF_GridGetCoord(srcGrid, localDE=lDE, staggerLoc=ESMF_STAGGERLOC_CENTER, coordDim=2, &
                            computationalLBound=clbnd, computationalUBound=cubnd, farrayPtr=farrayPtrYC, &
                            rc=localrc)
     if (localrc /=ESMF_SUCCESS) then
        rc=ESMF_FAILURE
        return
  ...
  ! Do regrid
  ! Only run ESMF_FieldRegrid() on PETs < 3
  if (localPet .lt. 3) then
     call ESMF_FieldRegrid(srcField, dstField, routeHandle, rc=localrc)
     if (localrc /=ESMF_SUCCESS) then
        rc=ESMF_FAILURE
        return
...
!            write(*,*) "relErr=",relErr,farrayPtr(i1,i2),xfarrayPtr(i1,i2)
 ...
 end subroutine test_regridPartialVM
 ...
 subroutine test_regridPerLocStatus(rc)
  ...
  if (PetCount .eq. 1) then
     ...
     if (statusPtr(3) .ne. ESMF_REGRIDSTATUS_MAPPED) correct=.false.
     if (statusPtr(4) .ne. ESMF_REGRIDSTATUS_DSTMASKED) correct=.false.
     ...
     if (statusPtr(9) .ne. ESMF_REGRIDSTATUS_SRCMASKED) correct=.false.
...
!  write(*,*) "MaxRelErr=",maxRelErr
 ...
 subroutine test_regrid_extrap_nearstod(rc)
  integer, intent(out)  :: rc
  logical :: correct
  integer :: localrc
  type(ESMF_Grid) :: srcGrid
  ...
  srcGrid=ESMF_GridCreateNoPeriDimUfrm(maxIndex=(/src_nx,src_ny/), &
       minCornerCoord=(/-50.0_ESMF_KIND_R8,-50.0_ESMF_KIND_R8/), &
       maxCornerCoord=(/48.0_ESMF_KIND_R8,48.0_ESMF_KIND_R8/), &
       staggerLocList=(/ESMF_STAGGERLOC_CENTER/), &
       rc=localrc)
  if (localrc /=ESMF_SUCCESS) then
    rc=ESMF_FAILURE
...
!  write(*,*) "avgRelErr=",avgRelErr/REAL(numPnts)
 ...
 end subroutine test_regrid_extrap_nearstod


 subroutine test_regrid_extrap_near_npnts(rc)
  integer, intent(out)  :: rc
  logical :: correct
  integer :: localrc
  type(ESMF_Grid) :: srcGrid
...
!  write(*,*) "avgRelErr=",avgRelErr/REAL(numPnts)
 ...
 end subroutine test_regrid_extrap_near_npnts


 subroutine test_regridPerLocStatusExtrap(rc)
        integer, intent(out)  :: rc
  logical :: correct
  integer :: localrc
  type(ESMF_Mesh) :: dstMesh
  ...
  integer, pointer :: elemIds(:),elemTypes(:),elemConn(:)
  integer :: numNodes, numElems
  integer :: numQuadElems,numTriElems, numTotElems, i
  ...
  if (PetCount .eq. 1) then
     ...
     if (statusPtr(7) .ne. ESMF_REGRIDSTATUS_MAPPED) correct=.false.
     if (statusPtr(8) .ne. ESMF_REGRIDSTATUS_MAPPED) correct=.false.
     if (statusPtr(9) .ne. ESMF_REGRIDSTATUS_EXMAPPED) correct=.false.
...
!  write(*,*) "avgRelErr=",avgRelErr/REAL(numPnts)
 ...
 end subroutine test_regrid_extrap_creep

 subroutine test_mesh_extrap_creep(rc)
...
!  write(*,*) localPet," Status Field=",statusPtr
  ...
  ! Check dst Field
  ! (Since we're extrapolating it should all be filled)
  ! Get Dst Field pointer
  call ESMF_FieldGet(dstField, 0, farrayPtr1D, computationalLBound=clbnd, &
                             computationalUBound=cubnd,  rc=localrc)
  if (localrc /=ESMF_SUCCESS) then
     rc=ESMF_FAILURE
     return
 ...
 end subroutine test_mesh_extrap_creep

 subroutine test_regridDisjointSD(rc)
        integer, intent(out)  :: rc
  logical :: correct
  integer :: localrc
  type(ESMF_Mesh) :: dstMesh
...
!  write(*,*) localPet," Status Field=",statusPtr
  ...
  if (PetCount .eq. 1) then
     ...
     if (statusPtr(3) .ne. ESMF_REGRIDSTATUS_OUTSIDE) correct=.false.
     if (statusPtr(4) .ne. ESMF_REGRIDSTATUS_DSTMASKED) correct=.false.
     if (statusPtr(5) .ne. ESMF_REGRIDSTATUS_OUTSIDE) correct=.false.
     if (statusPtr(6) .ne. ESMF_REGRIDSTATUS_OUTSIDE) correct=.false.
     if (statusPtr(7) .ne. ESMF_REGRIDSTATUS_OUTSIDE) correct=.false.
     if (statusPtr(8) .ne. ESMF_REGRIDSTATUS_OUTSIDE) correct=.false.
     if (statusPtr(9) .ne. ESMF_REGRIDSTATUS_OUTSIDE) correct=.false.
