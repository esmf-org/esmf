! $Id: ESMF_FieldCreateGetUTest.F90,v 1.37 2009/04/21 21:19:18 oehmke Exp $
!
...
!        call ESMF_Test((rc.eq.ESMF_SUCCESS), name, failMsg, result, ESMF_SRCLINE)
        ...
        write(failMsg, *) ""
        write(name, *) "Creating a 7D field using generic interface, arbitrary gridToFieldMap" // &
            " arbitrary distgridToGridMap, arbitrary haloWidth, ESMF_DATA_COPY"
        call ESMF_Test((rc.eq.ESMF_SUCCESS), name, failMsg, result, ESMF_SRCLINE)
        ...
        write(failMsg, *) ""
        write(name, *) "Creating a 7D field using generic interface, arbitrary gridToFieldMap" // &
            " arbitrary distgridToGridMap, arbitrary haloWidth, ESMF_DATA_COPY, CORNER STAGGER"
        call ESMF_Test((rc.eq.ESMF_SUCCESS), name, failMsg, result, ESMF_SRCLINE)
      ...
      write(failMsg, *) "Test unsuccessful"
      write(name, *) "Regrid with masks"

      ! initialize 
      rc=ESMF_SUCCESS
      
      ! do test
      call test_regridMask(rc)
        ...
        if(present(fieldget)) then
          if(fieldget) then
            ...
            if(present(copyflag)) then
                if(copyflag==ESMF_DATA_COPY) t = .true.
            endif
...
! Set the source grid coordinates to be a 0 to 360 grid
  ...
  ! Regrid store
  call ESMF_FieldRegridStore(field180, dstField360, &
          routeHandle=routeHandle, &
          regridMethod=ESMF_REGRID_METHOD_BILINEAR, &
          regridScheme=ESMF_REGRID_SCHEME_FULL3D, rc=localrc)
  if (localrc /=ESMF_SUCCESS) then
...
!               spherical=spherical_grid)
 ...
 end subroutine test_regridMask
