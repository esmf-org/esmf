// $Id: ESMC_LogErr.C,v 1.70.4.1 2006/03/29 21:49:57 svasquez Exp $
//
...
// into the object file for tracking purposes.
 static const char *const version = "$Id: ESMC_LogErr.C,v 1.70.4.1 2006/03/29 21:49:57 svasquez Exp $";
//----------------------------------------------------------------------------
//
...
//EOP
{
    ...
    if (ESMC_LogDefault.logNone == ESMF_TRUE) return true;
    ESMC_TimeStamp(&y,&mn,&d,&h,&m,&s,&ms);
	  ...
	  switch(msgtype)
	  {
		  case ESMC_LOG_INFO:
	  		  ...
	  		  y,mn,d,h,m,s,ms,"INFO     ",*pet_num,msg);
			    break;
		  case ESMC_LOG_WARN:
			    fprintf(ESMC_LogFile, "%.2d%.2d%.2d %.2d%.2d%.2d.%.6d %s PET%d %s\n",
	  		  y,mn,d,h,m,s,ms,"WARNING  ",*pet_num,msg);
			    break;
		  case ESMC_LOG_ERROR:
			    fprintf(ESMC_LogFile, "%.2d%.2d%.2d %.2d%.2d%.2d.%.6d %s PET%d %s\n",
	  		  y,mn,d,h,m,s,ms,"ERROR    ",*pet_num,msg);
			break;
	  }		
    fclose(ESMC_LogFile);
    return true;
	...
	switch(msgtype)
	{
		case ESMC_LOG_INFO:
			fprintf(ESMC_LogFile, "%.2d%.2d%.2d %.2d%.2d%.2d.%.6d %s PET%d %s %d %s %s\n",
	  		y,mn,d,h,m,s,ms,"INFO     ",*pet_num,FILE,LINE,method,msg);
			break;
		case ESMC_LOG_WARN:
			fprintf(ESMC_LogFile, "%.2d%.2d%.2d %.2d%.2d%.2d.%.6d %s PET%d %s %d %s %s\n",
	  		y,mn,d,h,m,s,ms,"WARNING  ",*pet_num,FILE,LINE,method,msg);
			break;
		case ESMC_LOG_ERROR:
			fprintf(ESMC_LogFile, "%.2d%.2d%.2d %.2d%.2d%.2d.%.6d %s PET%d %s %d %s %s\n",
	  		y,mn,d,h,m,s,ms,"ERROR    ",*pet_num,FILE,LINE,method,msg);
			break;
	}		
    fclose(ESMC_LogFile);
    return true;
