// $Id: ESMC_LogErr.C,v 1.44 2004/05/12 17:47:25 cpboulder Exp $
//
...
// into the object file for tracking purposes.
 static const char *const version = "$Id: ESMC_LogErr.C,v 1.44 2004/05/12 17:47:25 cpboulder Exp $";
//----------------------------------------------------------------------------
//
...
{
    strcpy(ESMF_Log.nameLogErrFile,filename);
    return ESMF_SUCCESS;
	...
	switch(logtype)
	{
		case ESMC_LOG_INFO:
			fprintf(ESMC_LogFile, "%.2d%.2d%.2d %.2d%.2d%.2d.%.6d %s %s %s\n",
	  		y,mn,d,h,m,s,ms,"INFO",modmeth,msg);
			break;
		case ESMC_LOG_WARN:
			fprintf(ESMC_LogFile, "%.2d%.2d%.2d %.2d%.2d%.2d.%.6d %s %s %s\n",
	  		y,mn,d,h,m,s,ms,"WARNING",modmeth,msg);
			break;
		case ESMC_LOG_ERROR:
			fprintf(ESMC_LogFile, "%.2d%.2d%.2d %.2d%.2d%.2d.%.6d %s %s %s\n",
	  		y,mn,d,h,m,s,ms,"ERROR",modmeth,msg);
			break;
	}		
    fclose(ESMC_LogFile);
    return ESMF_SUCCESS;
...
//EOP
{
	...
	{
		case ESMC_LOG_INFO:
			fprintf(ESMC_LogFile, "%.2d%.2d%.2d %.2d%.2d%.2d.%.6d %s %s %s %s\n",
	  		y,mn,d,h,m,s,ms,"INFO",module,method,msg);
			break;
		case ESMC_LOG_WARN:
			fprintf(ESMC_LogFile, "%.2d%.2d%.2d %.2d%.2d%.2d.%.6d %s %s %s %s\n",
	  		y,mn,d,h,m,s,ms,"WARNING",module,method,msg);
			break;
		case ESMC_LOG_ERROR:
			fprintf(ESMC_LogFile, "%.2d%.2d%.2d %.2d%.2d%.2d.%.6d %s %s %s %s\n",
	  		y,mn,d,h,m,s,ms,"ERROR",module,method,msg);
			break;
	}		
    fclose(ESMC_LogFile);
    return ESMF_SUCCESS;
...
//EOP
{
    ...
    int ok=0;
    int i=0;
    /*char fstr1[80]=__FILE__;
    char fstr2[32]="";
    int count1=strlen(fstr1);
    int count2=count1;
    while (fstr1[count2]!='/')
      count2--;
    int count3=0;
    for (count3=count2;count3=count1;count3++)
    	fstr2[count3-count2]=fstr1[count3];*/      
	...
	switch(logtype)
	{
		case ESMC_LOG_INFO:
			fprintf(ESMC_LogFile, "%.2d%.2d%.2d %.2d%.2d%.2d.%.6d %s %s %d %s\n",
	  		y,mn,d,h,m,s,ms,"INFO",FILE,LINE,msg);
			break;
		case ESMC_LOG_WARN:
			fprintf(ESMC_LogFile, "%.2d%.2d%.2d %.2d%.2d%.2d.%.6d %s %s %d %s\n",
	  		y,mn,d,h,m,s,ms,"WARNING",FILE,LINE,msg);
			break;
		case ESMC_LOG_ERROR:
			fprintf(ESMC_LogFile, "%.2d%.2d%.2d %.2d%.2d%.2d.%.6d %s %s %d %s\n",
	  		y,mn,d,h,m,s,ms,"ERROR",FILE,LINE,msg);
			break;
	}		
    fclose(ESMC_LogFile);
    return ESMF_SUCCESS;
	...
	switch(logtype)
	{
		case ESMC_LOG_INFO:
			fprintf(ESMC_LogFile, "%.2d%.2d%.2d %.2d%.2d%.2d.%.6d %s %s %d %s %s\n",
	  		y,mn,d,h,m,s,ms,"INFO",FILE,LINE,modmeth,msg);
			break;
		case ESMC_LOG_WARN:
			fprintf(ESMC_LogFile, "%.2d%.2d%.2d %.2d%.2d%.2d.%.6d %s %s %d %s %s\n",
	  		y,mn,d,h,m,s,ms,"WARNING",FILE,LINE,modmeth,msg);
			break;
		case ESMC_LOG_ERROR:
			fprintf(ESMC_LogFile, "%.2d%.2d%.2d %.2d%.2d%.2d.%.6d %s %s %d %s %s\n",
	  		y,mn,d,h,m,s,ms,"ERROR",FILE,LINE,modmeth,msg);
			break;
	}		
    fclose(ESMC_LogFile);
    return ESMF_SUCCESS;
	...
	switch(logtype)
	{
		case ESMC_LOG_INFO:
			fprintf(ESMC_LogFile, "%.2d%.2d%.2d %.2d%.2d%.2d.%.6d %s %s %d %s %s %s\n",
	  		y,mn,d,h,m,s,ms,"INFO",FILE,LINE,module,method,msg);
			break;
		case ESMC_LOG_WARN:
			fprintf(ESMC_LogFile, "%.2d%.2d%.2d %.2d%.2d%.2d.%.6d %s %s %d %s %s %s\n",
	  		y,mn,d,h,m,s,ms,"WARNING",FILE,LINE,module,method,msg);
			break;
		case ESMC_LOG_ERROR:
			fprintf(ESMC_LogFile, "%.2d%.2d%.2d %.2d%.2d%.2d.%.6d %s %s %d %s %s %s\n",
	  		y,mn,d,h,m,s,ms,"ERROR",FILE,LINE,module,method,msg);
			break;
	}		
    fclose(ESMC_LogFile);
    return ESMF_SUCCESS;
...
int ESMC_Log::ESMC_LogFoundError(
...
//EOP
{
	...
	if (rc!=ESMF_SUCCESS)
	{
		ESMC_LogWrite(msg,logtype,modmeth);
		result=ESMF_TRUE;
	}
	return result;
...
//EOP
{
	...
	if (rc!=ESMF_SUCCESS)
	{
		ESMC_LogWrite(msg,logtype,module,method);
		result=ESMF_TRUE;
	}
	return result;
...
void ESMC_Log::ESMC_LogSetFlush(
//
// !RETURN VALUE:
//  none
// !ARGUMENTS:
...
void ESMC_Log::ESMC_LogSetNotFlush(
//
// !RETUN VALUE:
//  none
// !ARGUMENTS        
...
void ESMC_Log::ESMC_LogSetNotVerbose(
// RETURN VALUE:
//  none
// !ARGUMENTS
//   none
...
void ESMC_Log::ESMC_LogSetHaltOnErr(
// RETURN VALUE:
//  none
// !ARGUMENTS
//   none
...
void ESMC_Log::ESMC_LogSetNotHaltOnErr(
//
// !RETURN VALUE:
//  none
// !ARGUMENTS
...
//
// !INTERFACE:
void ESMC_Log::ESMC_LogSetHaltOnWarn(
//
// !RETURN VALUE:
// none
// !ARGUMENTS
...
void ESMC_Log::ESMC_LogSetNotHaltOnWarn(
//
// !RETURN VALUE:
//  none
// !ARGUMENTS
