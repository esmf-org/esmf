...
!-------------------------------------------------------------------------------
 ...
 ! TODO: Eventually FIX THE BELOW TO DO A COMPLETE VERIFICATION OF READING THE 3x3 mesh
#if 0
  !-----------------------------------------------------------------------------
  !NEX_OFF_UTest
  write(name, *) "Test create 3x3 Mesh from file."
  write(failMsg, *) "Did not return ESMF_SUCCESS"
  ...
  call test_mesh_create_3x3_from_file(correct, rc)
...
subroutine  test_mesh_create_3x3_from_file(correct, rc)
  type(ESMF_Mesh) :: mesh
  logical :: correct
  integer :: rc
  ...
  if (petCount .eq. 1) then
   ...
   ! XMRKX !
   mesh=ESMF_MeshCreateFromFileNew("test_3x3_esmf.nc", &
        fileformat=ESMF_FILEFORMAT_ESMFMESH, &
        addUserArea=.true., &
        !       elementDistgrid=elemDistgrid, &
        rc=rc)
...
end subroutine test_mesh_create_3x3_from_file
#endif
