// $Id: ESMC_DDir.C,v 1.1 2007/08/07 17:48:00 dneckels Exp $
//
...
void DDir<HASH>::Print() {
  ...
  for (UInt i = 0; i < nent; i++) {
    ...
    std::cout << "P:" << rank << "(gid=" << de.gid << ", lid=" << de.origin_lid << ", op=" << de.origin_proc << std::endl;
  }
}
...
template class DDir<DDir_mod_hash>;
