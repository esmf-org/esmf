...
void extract_comm_answer( struct Data *data, struct Answer *answer, int my_proc, int nprocs);
void free_comm_data( struct Data *data, struct Data *my_data, struct Answer *true_answer);
void set_up_comm_from_send( struct Data *data, struct Data *my_data, int    blocked, int    my_proc, int    nprocs);
...
void print_plan( char *s, ZOLTAN_COMM_OBJ *plan,	/* communication data structure pointer */ int my_proc);
int read_comm_problem( FILE *in_file, struct Params *params, int *out_level, int my_proc);
double drandom();
void check_comm_info( ZOLTAN_COMM_OBJ *plan, struct Data *my_send_data, int nvals_recv, int my_proc);
