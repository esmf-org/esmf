...
namespace ESMCI {
  ...
  void _calc_elem_centroid(const MeshObj *elem, const MEField<>  *cfield, int sdim, double *cntr) {
  ...
  static bool _are_gids_less(const NBR_ELEM lhs, const NBR_ELEM rhs) {
    return (lhs.elem->get_id() < rhs.elem->get_id());
  }

  // Get the neighboring elements of a given element 
  void _get_neighbor_elems(const MeshObj *elem, const MEField<>  *cfield, const MEField<>  *mask_field, 
    ...
    while (nl != elem->Relations.end() && nl->obj->get_type() == MeshObj::NODE){
      ...
      while (el != node.Relations.end() && el->obj->get_type() == MeshObj::ELEMENT){
        ...
        _calc_elem_centroid(nbr_elem,cfield,3,tmp_ne.cntr);


        // See if it's in the list, if not add it
        std::vector<NBR_ELEM>::iterator lb = 
          std::lower_bound(nbrs->begin(), nbrs->end(), tmp_ne, _are_gids_less);
...
#define  MAX_NUM_POLY_COORDS_3D (3*MAX_NUM_POLY_NODES) 
    ...
    //    printf("Calculate 2nd Order Weights for 2D 3D grids\n");
    ...
    *src_elem_area=src_area;    
    ...
    for (int i=0; i<dst_elems.size(); i++) {
       ...
       // Init output variables to 0.0
       // (in case we leave early from the loop)
       (*valid)[i]=0;
