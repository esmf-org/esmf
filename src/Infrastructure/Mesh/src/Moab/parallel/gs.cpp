...
# undef LOOP
					...
					crystal_buf *lo, crystal_buf *hi)
  {
    ...
    lop = (uint*) lo->buf.ptr, hip = (uint*) hi->buf.ptr;
    while(src!=end) {
...
#define UINT_PER_LONG UINT_PER_X(slong)
						 ...
						 moab::TupleList &tl, unsigned pf)
  {
	...
	*buf++ = id; /* source */
	...
	if (tl.get_n()==tl.get_max()) {
	  ...
	  ri = tl.vi_wr + mi*tl.get_n(), rl = tl.vl_wr + ml*tl.get_n();
  ...
  ErrorCode gs_data::initialize(uint n, const long *label, 
				  const ulong *ulabel, uint maxv, const unsigned int nlabels, 
				  ...
				  crystal_data *crystal)
  {
	...
	if (label[nlabels*i]!=0) {
	  ...
	  nzi++, nzl+= nlabels, nzul+=nulabels, nonzero.inc_n();
	}
    }
	...
	for (j = 0; j < nulabels; j++)
	  pul[j]=nzul[j];
	pi[2]=1;
	pi+=3, pl+=nlabels; pul+=nulabels; primary.inc_n();
      }
    }

    /* calculate size of local condense map */
    {
      uint i, count=1; sint *pi=primary.vi_wr;
	...
	for (;pl2[0]==lbl; pi2+=3,pl2+=nlabels,pul2+=nulabels) {
	  if (shared.get_n()+2 > shared.get_max())
	    shared.resize((shared.get_max() ? shared.get_max()+(shared.get_max()+1)/2+1 : 2)),
	      si=shared.vi_wr+shared.get_n()*3, sl=shared.vl_wr+shared.get_n()*nlabels,
	  ...
	  for (j = 0; j < nulabels; j++)
	    sul[j] = pul2[j];
	  si+=3, sl+=nlabels, sul+=nulabels, shared.inc_n();
	  si[0] = pi2[0];
	  si[1] = pi1[0];
	  si[2] = pi2[1];
	  for (j = 0; j < nlabels; j++)
    ...
    /* count partner procs */
    {
      uint i, count=0; sint proc=-1,*si=shared.vi_wr;
