...
void get_global_ids(const std::vector<MeshObj::id_type> &current_ids,
...
void Mesh::assign_new_ids() {
  ...
  for (UInt tp = 0; tp < NumMeshObjTypes; tp++) {
    ...
    get_global_ids(used_ids, needed_ids);
...
void Mesh::Commit() {
  ...
  MeshDB::Commit(FieldReg::Numfields(), FieldReg::ListOfFields());
