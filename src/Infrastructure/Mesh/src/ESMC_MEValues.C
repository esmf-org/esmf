      ...
      const Mapping<typename ME2MPTraits<METRAITS>::value> *map) 
{
  ...
  buf_size += round_to_dword(sizeof(buf_manage));

  if (update_flag & MEV::update_sf) {
    buf_size += round_to_dword(ltotal_nqpoints*lmeptr->num_functions()*sizeof(double));
  }
  if (update_flag & MEV::update_sfg) {
    // param_sfg
    buf_size += round_to_dword(ltotal_nqpoints*lmeptr->num_functions()*topo->parametric_dim*sizeof(double));
    // sfg
    buf_size += round_to_dword(ltotal_nqpoints*sdim*lmeptr->num_functions()*sizeof(double));
  }
  if (update_flag & MEV::update_jxw) {
    buf_size += round_to_dword(ltotal_nqpoints*sizeof(double));
  }
  if (update_flag & MEV::update_map) {
    buf_size += round_to_dword(lcmeptr->num_functions()*sdim*sizeof(double));
  }
