...
// Write mesh data to stream, taking into account the correct typeid
template<>
void append_data_from_Array<ESMC_R8>(std::vector<const MeshObj *>::iterator ni, std::vector<const MeshObj *>::iterator ne, std::map<MeshObj::id_type, int> gid_to_index, 
                        ESMCI::Array *array, std::ofstream &out) {
...
template<>
void append_data<double>(std::vector<const MeshObj *>::iterator ni, std::vector<const MeshObj *>::iterator ne, const _field &llf, UInt d, std::ofstream &out) {
  double zero=0.0;
  out << "LOOKUP_TABLE default" << std::endl;
  for (; ni != ne; ++ni) {
    if (llf.OnObj(**ni)) {
      // It seems to irritate visit if a |value| < 1.0E-300, so round down to zero if that's the case
...
static void write_data(iter ni, iter ne, const FIELD &llf, const std::string &vname, UInt d, std::ofstream &out) {
  ...
  static bool _is_data_index_less(const MeshObj *lhs, const MeshObj *rhs)\
  {
    return (lhs->get_data_index() < rhs->get_data_index());
  }
  ...
  // TODO add version
  // Header Max 256 chars
  out << "This file was generated by ESMF : File Version=1 : Mesh Info coordsys=1 pdim="<<mesh.parametric_dim()<<" sdim="<<mesh.orig_spatial_dim<< std::endl;
  out << "ASCII" << std::endl;
  out << "DATASET UNSTRUCTURED_GRID" << std::endl;
  out << "POINTS " << mesh.num_nodes() << " double" << std::endl;


  // Get lists of nodes sorted by data index
  // (So they will be in original creation order)
  std::vector<const MeshObj *> sorted_nodes;
  sorted_nodes.reserve(mesh.num_nodes());
  Mesh::const_iterator ni = mesh.node_begin(), ne = mesh.node_end();
  for (; ni != ne; ++ni) {
    sorted_nodes.push_back(&(*ni));
  }
  std::sort(sorted_nodes.begin(), sorted_nodes.end(), _is_data_index_less);

  // Get lists of elems sorted by data index
  // (So they will be in original creation order)
  std::vector<const MeshObj *> sorted_elems;
  sorted_elems.reserve(mesh.num_elems());
  Mesh::const_iterator ei = mesh.elem_begin(), ee = mesh.elem_end();
  for (; ei != ee; ++ei) {
    sorted_elems.push_back(&(*ei));
  }
  std::sort(sorted_elems.begin(), sorted_elems.end(), _is_data_index_less);
  

  // Map from ids to where they are in output list
  std::map<MeshObj::id_type, int> id2ord;

  // Output points
  {
    MEField<> &coord = *mesh.GetCoordField();
    std::vector<const MeshObj *>::iterator ni=sorted_nodes.begin(), ne=sorted_nodes.end();
...
//std::cout << "quad!@! " << mesh.parametric_dim() << std::endl;
     ...
     for (; nv != ne; ++nv) {
       ...
       if (mf.Output() && mf.is_elemental()) {
         ...
         for (UInt d = 0; d < mf.dim(); d++) {
           ...
           const _field &llf = mf();
           write_data(sorted_elems.begin(), sorted_elems.end(), llf, vname, d, out);
  ...
  // Now Arrays stored on elems
  {
    ...
    for (int i=0; i<num_elemArrays; i++) {
      ...
      // Write data to file
      write_data_from_Array(sorted_elems.begin(), sorted_elems.end(), gid_to_index, elemArrays[i], vname, out);
  ...
  // Save off the node owner
  {
    // Node owners
    out << "SCALARS " << "_NODE_OWNER" << " long 1" << std::endl;
    out << "LOOKUP_TABLE default" << std::endl;
    std::vector<const MeshObj *>::iterator ni=sorted_nodes.begin(), ne=sorted_nodes.end();
    for (UInt i = 0; ni != ne; ++ni) {
      const MeshObj &node = **ni;
  ...
  // Now node variables
  {
     ...
     for (; nv != ne; ++nv) {
       ...
       bool output=mf.Output(); // output if user requests it
       if (mf.name() == "orig_coordinates") output=true;  // Output if it's node original coords
       if (mf.name() == "node_mask_val") output=true;  // Output if it's node mask value

           /// Output
       if (output && mf.is_nodal()) {
         ...
         for (UInt d = 0; d < mf.dim(); d++) {
           ...
           const _field &llf = mf();
           write_data(sorted_nodes.begin(), sorted_nodes.end(), llf, vname, d, out);
  ...
  // Now Arrays stored on nodes
  {
    ...
    for (int i=0; i<num_nodeArrays; i++) {
      ...
      // Write data to file
      write_data_from_Array(sorted_nodes.begin(), sorted_nodes.end(), gid_to_index, nodeArrays[i], vname, out);
 ...
 */
  ...
  FileManager fp(filename.c_str());
  {
    ...
    char linebuf[4096];
    float version;
    ret = fscanf(fp(), "# vtk DataFile Version %f\n", &version);
    ThrowRequire(ret == 1);

    // Get mesh info
    int fv;
    ret = fscanf(fp(), "This file was generated by ESMF : File Version=%d : Mesh Info coordsys=%d pdim=%d sdim=%d\n", &fv,&coord_sys,&pdim,&sdim);
    ThrowRequire(ret == 4); // return matches number of values succesfully filled
  ...
  //std::cout << "npoints = " << npoints << std::endl;
    ...
    /*------------------------------------------------------------*/
    // Skip past the connectivity for elements
    /*------------------------------------------------------------*/
    UInt nnode;
    for (UInt i = 0; i < nelem; i++) {
      ...
      for (UInt n = 0; n < nnode; n++) {
        int tmp;
        ThrowRequire(1 == fscanf(fp(), "%u ", &tmp));
      } 
      fscanf(fp(), "\n");
    }
    ...
    // Set some output arguments based on interal vars
    num_nodes = npoints;
    num_elems = nelem;
    conn_size = width-nelem;  // width includes an entry for num_nodes at beginning
                              // of each node list.  We will not forward this.
    ...
    while (!done) {
      ...
      // Check for elem area
      if (data_type == VTK_CELL_DATA && std::string(vname) == "elem_area") {
        elem_area_present=true;

      } else if (data_type == VTK_CELL_DATA && std::string(vname) == "elem_mask_val") {
        elem_mask_present=true;

      } else if (data_type == VTK_CELL_DATA && std::string(vname) == "elem_orig_coordinates_0") {
        elem_coords_present=true;

      } else if (data_type == VTK_NODE_DATA && std::string(vname) == "node_mask_val") {     
...
/**
 *  Get information to create the body of the mesh
 */
void ReadVTKMeshBodyComplete(const std::string &filename, int *nodeId, double *nodeCoord, int *nodeOwner,
                             int *elemId, int *elemType, int *elemConn, 
                             // Optional args
                             int *nodeMask, int *elemMask, double *elemArea, double *elemCoords) {

  FileManager fp(filename.c_str());

  // Mesh info
  int coord_sys,pdim,orig_sdim;;
  UInt num_nodes, num_elems;
  ...
  //std::cout << "npoints = " << npoints << std::endl;
    ...
    /*------------------------------------------------------------*/
    // Now read the coords, if Cartesian
    /*------------------------------------------------------------*/
    // If Cartesian, just use the node coords
    if (coord_sys == 0) {
      for (UInt i = 0; i < npoints; i++) {
        int t_i = 3*i;
        ThrowRequire(3 == fscanf(fp(), "%lf %lf %lf\n", &nodeCoord[t_i], &nodeCoord[t_i+1], &nodeCoord[t_i+2]));
...
//std::cout << "Read " << data[i] << std::endl;
    ...
    if (data_type == VTK_CELL_DATA && std::string(vname) == "_ELEM_NUM") {
      ...
      // If elemMask exists, then fill it
      if (elemMask != NULL) { 
        for (UInt i = 0; i < data.size(); ++i) elemMask[i] = (int)(data[i]+0.0001);
      }

    } else if (data_type == VTK_CELL_DATA && std::string(vname) == "elem_area") {

      // If elemArea exists, then fill it
      if (elemArea != NULL) {
        for (UInt i = 0; i < data.size(); ++i) elemArea[i] = data[i];
      }

    } else if (data_type == VTK_CELL_DATA && std::string(vname) == "elem_orig_coordinates_0") {

      // If not Cartesian and elemCoords exists, put into elemCoords
      if ((coord_sys != 0) && (elemCoords != NULL)) { 
        for (UInt i = 0; i < data.size(); ++i) elemCoords[orig_sdim*i] = data[i];
      }

    } else if (data_type == VTK_CELL_DATA && std::string(vname) == "elem_orig_coordinates_1") {

      // If not Cartesian and elemCoords exists, put into elemCoords
      if ((coord_sys != 0) && (elemCoords != NULL)) { 
        for (UInt i = 0; i < data.size(); ++i) elemCoords[orig_sdim*i+1] = data[i];
      }

    } else if (data_type == VTK_CELL_DATA && std::string(vname) == "elem_orig_coordinates_2") {

      // If not Cartesian and elemCoords exists, put into elemCoords
      if ((coord_sys != 0) && (elemCoords != NULL)) { 
        if (orig_sdim < 3) Throw() << "Inconsistent file: sdim<3, but contains 3rd coord";
        for (UInt i = 0; i < data.size(); ++i) elemCoords[orig_sdim*i+2] = data[i];
      }
    } else if (data_type == VTK_CELL_DATA && std::string(vname) == "elem_coordinates_0") {
    ...
    } else if (data_type == VTK_CELL_DATA && std::string(vname) == "elem_coordinates_1") {
    ...
    } else if (data_type == VTK_CELL_DATA && std::string(vname) == "elem_coordinates_2") {
    ...
    } else if (data_type == VTK_NODE_DATA && std::string(vname) == "_NODE_OWNER") {

      for (UInt i = 0; i < data.size(); ++i) nodeOwner[i] = (int)(data[i]+0.0001);

    } else if (data_type == VTK_NODE_DATA && std::string(vname) == "orig_coordinates_0") {

      // If not Cartesian, put into nodeCoords
      if (coord_sys !=0) { 
        for (UInt i = 0; i < data.size(); ++i) nodeCoord[orig_sdim*i] = data[i];
      }

    } else if (data_type == VTK_NODE_DATA && std::string(vname) == "orig_coordinates_1") {

      // If not Cartesian, put into nodeCoords
      if (coord_sys !=0) { 
        for (UInt i = 0; i < data.size(); ++i) nodeCoord[orig_sdim*i+1] = data[i];
      }

    } else if (data_type == VTK_NODE_DATA && std::string(vname) == "orig_coordinates_2") {

      // If not Cartesian, put into nodeCoords
      if (coord_sys !=0) { 
        if (orig_sdim < 3) Throw() << "Inconsistent file: sdim<3, but contains 3rd coord";
        for (UInt i = 0; i < data.size(); ++i) nodeCoord[orig_sdim*i+2] = data[i];
      }
    } else if (data_type == VTK_NODE_DATA && std::string(vname) == "node_mask_val") {

      // If nodeMask exists, then fill it
      if (nodeMask != NULL) {
        for (UInt i = 0; i < data.size(); ++i) nodeMask[i] = (int)(data[i]+0.0001);
      }
    } 
