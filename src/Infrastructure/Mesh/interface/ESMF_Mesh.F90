...
!    endif
    ...
    if (present(elementDistgrid)) then
       ...
       call ESMF_DistGridGetNumIds(elementDistgrid, numElem, rc=localrc)
       if (ESMF_LogFoundError(localrc, ESMF_ERR_PASSTHRU, &
            ESMF_CONTEXT, rcToReturn=rc)) return
    else
...
!------------------------------------------------------------------------------
    ...
    call C_ESMC_MeshGetOwnedElemCount(ESMF_MeshCreateFromPointer%this, &
                                      numElem, localrc)
    if (ESMF_LogFoundError(localrc, ESMF_ERR_PASSTHRU, &
        ESMF_CONTEXT, rcToReturn=rc)) return
    
    call C_ESMC_MeshGetOwnedNodeCount(ESMF_MeshCreateFromPointer%this, &
                                      numNode, localrc)
    if (ESMF_LogFoundError(localrc, ESMF_ERR_PASSTHRU, &
        ESMF_CONTEXT, rcToReturn=rc)) return
...
!------------------------------------------------------------------------------
    ...
    call C_ESMC_MeshGetOwnedElemCount(ESMF_MeshCreateFromIntPtr%this, &
                                      numElem, localrc)
    if (ESMF_LogFoundError(localrc, ESMF_ERR_PASSTHRU, &
        ESMF_CONTEXT, rcToReturn=rc)) return
    
    call C_ESMC_MeshGetOwnedNodeCount(ESMF_MeshCreateFromIntPtr%this, &
                                      numNode, localrc)
    if (ESMF_LogFoundError(localrc, ESMF_ERR_PASSTHRU, &
        ESMF_CONTEXT, rcToReturn=rc)) return
...
!------------------------------------------------------------------------------
    ...
    call c_esmc_meshgetelemcount(mesh%this, numElem, localrc)
    if (ESMF_LogFoundError(localrc, ESMF_ERR_PASSTHRU, &
        ESMF_CONTEXT, rcToReturn=rc)) return

    ! Allocate array to hold split areas
    allocate(splitAreaList(numElem))
