! $Id: ESMF_MeshEx.F90,v 1.18 2009/09/23 23:13:01 oehmke Exp $
!
...
! associated with that node, but not on any other PET (a node can't be on a PET without an element ""home"). When a Field is
! created on a Mesh (i.e. on  the Mesh nodes), on each PET the Field is only created on the nodes which are owned by that PET.
...
! (e.g. a triangle vs. a quadralateral). The range of choices for the topologyu of the elements in a Mesh are restricted by the 
...
! any combination of elements appropriate to its dimension. The element connectivity indicates which elements are to be connected together to
...
!BOE
!\subsubsection{Example: Creating a Small Single PET Mesh in one Step}
!
...
! seen in Figure~\ref{fig:mesh:2D1PET}. Note that this Mesh consists of 9 nodes and 5 elements, where the elements are a mixture of 
...
!
!\end{verbatim}
!\caption{A small 2D Mesh used to illustrate Mesh creation.}
!\label{fig:mesh:2D1PET}
!\end{figure}
!\end{center}
!
! 
...
!BOE
!\subsubsection{Example: Creating a Small Single PET Mesh in Three Steps}
!
...
! the same one as in the last example (Figure~\ref{fig:mesh:2D1PET}), but the three step process allows the creation to occur in 
...
! previous two examples (See Figure~\ref{fig:mesh:2D1PET}), however, in this case the Mesh is broken up across 4 PETs. 
! Figure~\ref{fig:mesh:2D4PET} illustrates the distribution of the Mesh across the PETs. As in the previous diagram, the node ids are in
! the corners of the elements and the element ids are in the centers. In this figure the '*' character indicates a node which
! is owned by another PET. The following code illustrates the creation of this Mesh in parallel.  
!
!\begin{center}
!\begin{figure}[h!]
!\center
...
!BOE
!\subsubsection{Example: Creating a Small Mesh on 4 PETs in Three Steps}
!
! This example is intended to illustrate the creation of a small Mesh on multiple PETs using the more memory efficient 
! three step approach. This example creates the same small 2D Mesh as the 
! previous examples (See Figure~\ref{fig:mesh:2D1PET}) and like in the previous example the Mesh is broken up across 4 PETs 
! (See Figure~\ref{fig:mesh:2D4PET}). The following code illustrates the creation of this Mesh in parallel in three steps.
!
!EOE
...
!BOC
 ...
 ! Break up what's being set by PET for nodes
 if (localPET .eq. 0) then !!! This part only for PET 0
    ! Set number of nodes
     numNodes=4
 ...
 ! Break up what's being set by PET for elements
 if (localPET .eq. 0) then !!! This part only for PET 0
    ! Set the number of each type of element, plus the total number.
    numQuadElems=1
    numTriElems=0
  ...
  ! After adding the elements we are through with the element arrays, 
  ! so they may be deallocated.
  deallocate(elemIds)
  deallocate(elemTypes)
  deallocate(elemConn)
...
! leaves the distgrid information. This significantly reduces the memory used by a Mesh, but restricts what can be done with
! it. Once this subroutine has been called on a Mesh object any Field built on 
! the object can  no longer be used as part of an {\tt ESMF\_FieldRegridStore()} call. However, because the distgrid 
! information is still part of the Mesh, Fields built on such a Mesh can still be part of an {\tt ESMF\_FieldRegridRun()}
...
!BOC
   ...
   ! of an ESMF_FieldRegridRun() call
