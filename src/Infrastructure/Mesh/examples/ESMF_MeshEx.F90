...
!BOE
!\subsubsection{Create a small Mesh on 4 PETs using easy element method}
...
! The figure above illustrates the distribution of the Mesh across the PETs. As in the previous diagrams, the element ids are in the centers. 
! However, unlike some examples of 
! Mesh creation, here the user doesn't specify node ids or ownership, so that information is replaced by a "*".
! Note that in the example below the user doesn't specify the element ids. In this case, they are assigned sequentially
! through the local elements on each PET starting with 1 for the first element on PET 0. It isn't shown here, but there is
...
! only needs to specify the element type once and the element corner coordinate array has an extra dimension so that the corners for all
! the elements are not collapsed into a 1D list. 
! The figure above illustrates the distribution of the Mesh across the PETs. As in the previous diagrams, the element ids are in the centers. 
! However, unlike some examples of 
! Mesh creation, here the user doesn't specify node ids or ownership, so that information is replaced by a "*".
! Note that in the example below the user doesn't specify the element ids. In this case, they are assigned sequentially
