//==============================================================================
// $Id: ESMC_RendEx.C,v 1.1 2007/08/24 19:15:44 dneckels Exp $
//
...
void fill_src(const Mesh &mesh, MEField<> *s) {
  ...
  for (; ni != ne; ++ni) {
    ...
    data[0] = sin(6*c[0]+3*c[1])*cos(4*c[2]);
  }
}
...
int main(int argc, char *argv[]) {
  ...
  s = srcmesh.RegisterField("source", MEFamilyStd::instance(), MeshObj::ELEMENT, ctxt, 1, true);
  d = dstmesh.RegisterField("dest", MEFamilyStd::instance(), MeshObj::ELEMENT, ctxt, 1, true);
  ...
  if (Par::Rank()) std::cout << "Calling Zoltan!!" << std::endl;
