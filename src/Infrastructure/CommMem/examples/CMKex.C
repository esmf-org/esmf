...
// ESMC CommMem application test program
...
//
// on halem, build with
// cxx -w1 -std strict_ansi -pthread -I/u1/eschwab/esmf/src/include -I/u1/eschwab/esmf/src/Infrastructure/Base/include -I../include -o petest8 petest8.C ESMC_PE.o ESMC_Machine.o ESMC_Comm.o ESMC_DE.o ESMC_PEList.o ESMC_Layout.o /u1/eschwab/esmf/src/Infrastructure/Base/src/ESMC_Base.o -lm -lmpi
//
// then run with
// bsub -P "hp606" -q general -n 8 prun -m cyclic -n 2 -N 2 ./petest8
//-----------------------------------------------------------------------------
...
int main(int argc, char **argv)
{
  ...
  for (int i=1; i<ESMC_COMM_NTHREADS; i++) {
    ...
    //std::cout << "thread " << i << " started, tid=" << ESMC_Comm_tid[i] << std::endl;
  }
...
//std::cout << "I am tid, detype " << pthread_self() <<
             //", " << detype << std::endl;
...
//std::cout << "comm group size = " << nDEs << "\n";
  de.ESMC_DEGetESMFID(&myDEid);
//std::cout << "myDEid = " << myDEid << "\n";
...
//std::cout << "mycpuid, mynodeid = " << mycpuid << ", " << mynodeid << "\n";
  ...
  for(int i=0; i<nDEs; i++) {
    ...
    if (myDEid == 0) {
      std::cout << "(peid,cpuid,nodeid) = "  << mypeid   << " "
                                             << mycpuid  << " "
                                             << mynodeid << " \n";
    }
  }
  ...
  for(int r=0; r < 10000000; r++) {
    ...
    if(r%1000000 == 0) {
      ...
      pe.ESMC_PEGetCpuID(&mycpuid);
      pe.ESMC_PEGetNodeID(&mynodeid);
     //std::cout << "tid, mycpuid, mynodeid = " << pthread_self() << ", " << mycpuid << ", " << mynodeid << "\n";
     }
  }
#endif
