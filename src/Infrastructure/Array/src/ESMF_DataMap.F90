! $Id: ESMF_DataMap.F90,v 1.1 2003/07/10 18:46:14 nscollins Exp $
!
...
! !PRIVATE MEMBER FUNCTIONS:
       ...
       module procedure ESMF_DataMapCreateMatchGrid
...
! !PRIVATE MEMBER FUNCTIONS:
        ...
        module procedure ESMF_DataMapConstructMatchGrid
...
!BOP
! !INTERFACE:
      function ESMF_DataMapCreateMatchGrid(grid, relloc, rc)
!
! !RETURN VALUE:
      type(ESMF_DataMap) :: ESMF_DataMapCreateMatchGrid
!
!
! !ARGUMENTS:
...
! !REQUIREMENTS:
        ...
        ! Initialize pointer
        nullify(dmp) 
        nullify(ESMF_DataMapCreateMatchGrid%dmp)
        ...
        ! Allocate space for DataMap object and call Construct method to initalize
        allocate(dmp, stat=status)
        if (status .ne. 0) then         ! this is a fortran rc, NOT an ESMF rc
          print *, "DataMap allocation error"
          return
        ...
        call ESMF_DataMapConstructMatchGrid(dmp, grid, relloc, status)
        if (status .ne. ESMF_SUCCESS) then
          print *, "DataMap construction error"
          return
        endif
        ...
        ! Set return values
        ESMF_DataMapCreateMatchGrid%dmp => dmp
        if (rcpresent) rc = ESMF_SUCCESS

        end function ESMF_DataMapCreateMatchGrid


!------------------------------------------------------------------------------
!BOP
! !INTERFACE:
      function ESMF_DataMapCreateFastest(grid, rc);

! !RETURN VALUE:
      type(ESMF_DataMap) :: ESMF_DataMapCreateFastest  
!
! !ARGUMENTS:
      type(ESMF_Grid), pointer :: grid               ! grid to match
...
!       query the Grid to find out the rank
!       ESMF_GridGetRank(grid, datamap%gridrank, status)
        status = ESMF_SUCCESS
!       remove the previous line when the Grid routine exists
        if (status .ne. ESMF_SUCCESS) then
          print *, "Error getting rank from Grid"
          return
        endif

!       set up the datamap so the index orders match
        datamap%dim_order = 0
        do i=1,datamap%gridrank
            datamap%dim_order(i) = i
        enddo
