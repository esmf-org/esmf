! $Id: ESMF_FieldBundleRedistHelpers.F90,v 1.7.4.1 2010/02/05 19:56:33 svasquez Exp $
!
...
function CreateDataField(name, igrid, layout, relloc, r4value, r8value, rc)
  ...
  if (use_r8) then
      call ESMF_ArraySpecSet(as, rank=2, typekind=ESMF_TYPEKIND_R8, rc=status)
  else
      call ESMF_ArraySpecSet(as, rank=2, typekind=ESMF_TYPEKIND_R4, rc=status)
  endif
  if (ESMF_LogMsgFoundError(status, &
                            ESMF_ERR_PASSTHRU, &
                            ESMF_CONTEXT, rc)) goto 10
