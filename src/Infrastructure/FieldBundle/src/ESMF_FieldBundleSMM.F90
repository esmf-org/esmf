! $Id: ESMF_FieldBundleSMM.F90,v 1.1 2008/05/22 17:28:25 feiliu Exp $
!
...
!------------------------------------------------------------------------------
    ...
    end interface ESMF_FieldBundleSMMStore
!------------------------------------------------------------------------------
contains
...
!EOPI
        ...
        do i = 1, fcount
            ...
            ! Rely on underlying ArraySMM to perform sanity checking of the other parameters 
            call ESMF_FieldSMMStore(srcField, dstField, routehandle, factorList, & 
...
! patches within the DistGrid or by user-supplied arbitrary sequence indices. See 
! section \ref{Array:SparseMatMul} for details on the definition of {\em sequence indices}. 
! SMMribution corresponds to an identity mapping of the source FieldBundle vector to 
! the destination FieldBundle vector. 
!  
! Source and destination Fields may be of different <type><kind>. Further source 
! and destination Fields may differ in shape, however, the number of elements 
...
!EOPI
        ...
        do i = 1, 1 !fcount
            ...
            call ESMF_FieldSMMStore(srcField, dstField, routehandle, rc=localrc)
            if (ESMF_LogMsgFoundError(localrc, & 
                ESMF_ERR_PASSTHRU, & 
                ESMF_CONTEXT, rc)) return 
        enddo
