...
!  ! data is relative to the igrid, and any interleaving info needed.
      ...
      private
        ...
        type(ESMF_StaggerLoc) :: staggerloc             ! data item loc/cell
        ESMF_INIT_DECLARE
...
!EOP
        ...
        if (present(staggerloc)) staggerloc = fielddatamap%staggerloc
...
!EOP
        ...
        if (present(staggerloc)) fielddatamap%staggerloc = staggerloc
...
!           predefined values see Section \ref{sec:opt:staggerloc}.
!           To create a custome stagger location see Section
!           \ref{sec:usage:staggerloc:adv}.  The default is
!           {\tt ESMF\_STAGGERLOC\_CENTER}.
! \item [{[rc]}]
!       Return code; equals {\tt ESMF\_SUCCESS} if there are no errors.
! \end{description}
!
!EOP
        ...
        ! assume scalar data and use the staggerloc the caller gave
        fielddatamap%rankLength = 0

        fielddatamap%staggerloc = ESMF_STAGGERLOC_CENTER
        if (present(staggerloc)) fielddatamap%staggerloc = staggerloc
...
!EOP
        ...
        if (present(staggerloc)) then
          fielddatamap%staggerloc = staggerloc
        else
          fielddatamap%staggerloc = ESMF_STAGGERLOC_CENTER
        endif
...
!     \item [{[iospec]}]
!           File specification. ! NOT IMPLEMENTED
!     \item [{[rc]}]
!           Return code; equals {\tt ESMF\_SUCCESS} if there are no errors.
!       \end{description}
!
...
!EOPI
      ...
      call c_ESMC_ArrayDataMapSerialize(fielddatamap%adm%status, &
                                 ...
                                 fielddatamap%adm%dataNonIGridCounts, &
                                 buffer(1), length, offset, localrc)
      if (ESMF_LogMsgFoundError(localrc, &
                                 ESMF_ERR_PASSTHRU, &
                                 ESMF_CONTEXT, rc)) return
