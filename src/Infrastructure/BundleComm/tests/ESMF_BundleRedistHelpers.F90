! $Id: ESMF_BundleRedistHelpers.F90,v 1.19 2007/03/31 02:24:30 cdeluca Exp $
!
...
function CreateDataField(name, grid, layout, relloc, r4value, r8value, rc)
  ...
  if (use_r8) then
      call ESMF_ArraySpecSet(as, rank=2, kind=ESMF_TYPEKIND_R8, rc=status)
  else
      call ESMF_ArraySpecSet(as, rank=2, kind=ESMF_TYPEKIND_R4, rc=status)
  endif
  if (ESMF_LogMsgFoundError(status, &
                            ESMF_ERR_PASSTHRU, &
                            ESMF_CONTEXT, rc)) goto 10
