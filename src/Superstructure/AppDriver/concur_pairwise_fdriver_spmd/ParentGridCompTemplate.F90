! $Id: ParentGridCompTemplate.F90,v 1.2.4.1 2008/05/06 04:31:39 cdeluca Exp $
!
...
!  Components:
!  two Gridded Components which run concurrently and perform a computation 
...
!\begin{verbatim}
    ...
    subroutine my_init(gcomp, importState, exportState, parentclock, rc)
      ...
      integer :: i, parentpetcount, split
      ...
      call ESMF_VMGet(parentvm, petCount=parentpetcount, rc=rc)

      ! Divide list in 2, rounding up if odd number of PETs
      split = (parentpetcount + 1) / 2
      ...
      call ESMF_StateAdd(Cplimp, G1imp, rc)
      call ESMF_StateAdd(Cplimp, G2imp, rc)
      call ESMF_StateAdd(Cplexp, G1exp, rc)
      call ESMF_StateAdd(Cplexp, G2exp, rc)
    ...
    subroutine my_run(gcomp, importState, exportState, parentclock, rc)
      ...
      ! This is where the main run loop would be.

      ! Now run the subcomponents concurrently.
