% $Id: AppDriver_desc.tex,v 1.16 2007/06/22 23:21:43 cdeluca Exp $
%
...
In a serial execution model every PET executes the same InternGridded 
Component code until it has produced data needed by another 
InternGridded Component, and then all PETs change to running the next 
InternGridded Component or Coupler Component.  This has the appeal of 
simplicity of data consumption and production: when a InternGridded 
Component starts all required data is available for use, and when 
a InternGridded Component finishes all data produced is ready for consumption 
by the next InternGridded Component.  This approach also has
the possibility of less data movement if the interngridding and
...
InternGridded Components and all InternGridded Components are active at the 
same time.  Data exchange must be coordinated between InternGridded 
Components so that data deadlock does not occur.  This strategy 
has the advantage of allowing coupling to other InternGridded Components 
...
InternGridded Component and putting it into the form expected by another 
InternGridded Component.  This might include regridding, change of units, 
...
it for use by another single InternGridded Component.  This simplifies the 
