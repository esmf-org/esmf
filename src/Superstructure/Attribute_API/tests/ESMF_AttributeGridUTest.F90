! $Id: ESMF_AttributeGridUTest.F90,v 1.2 2008/08/08 15:25:27 rokuingh Exp $
!
...
!-------------------------------------------------------------------------
    ...
    !-------------------------------------------------------------------------
      ...
      !EX_UTest
      ! Destroy an Attribute on a Grid Test
      call ESMF_AttributeDestroy(grid, name="AttrI4", rc=rc)
      write(failMsg, *) "Did not return ESMF_SUCCESS"
      write(name, *) "Destroying an ESMF_I4 Attribute on a Grid Test"
      call ESMF_Test((rc==ESMF_SUCCESS), name, failMsg, result, ESMF_SRCLINE)
      !------------------------------------------------------------------------
    ...
    !-------------------------------------------------------------------------
      ...
      call ESMF_AttributeSet(grid, name="AttrI4l", count=count, &
      ...
      call ESMF_AttributeGet(grid, name="AttrI4l", count=count, &
      ...
      !EX_UTest
      ! Destroy an Attribute on a Grid Test
      call ESMF_AttributeDestroy(grid, name="AttrI4l", rc=rc)
      write(failMsg, *) "Did not return ESMF_SUCCESS"
      write(name, *) "Destroying an ESMF_I4l Attribute on a Grid Test"
      call ESMF_Test((rc==ESMF_SUCCESS), name, failMsg, result, ESMF_SRCLINE)
      !------------------------------------------------------------------------
    ...
    !-------------------------------------------------------------------------
      ...
      !EX_UTest
      ! Destroy an Attribute on a Grid Test
      call ESMF_AttributeDestroy(grid, name="AttrI8", rc=rc)
      write(failMsg, *) "Did not return ESMF_SUCCESS"
      write(name, *) "Destroying an ESMF_I8 Attribute on a Grid Test"
      call ESMF_Test((rc==ESMF_SUCCESS), name, failMsg, result, ESMF_SRCLINE)
      !------------------------------------------------------------------------
    ...
    !-------------------------------------------------------------------------
      ...
      call ESMF_AttributeSet(grid, name="AttrI8l", count=count, &
      ...
      call ESMF_AttributeGet(grid, name="AttrI8l", count=count, &
      ...
      !EX_UTest
      ! Destroy an Attribute on a Grid Test
      call ESMF_AttributeDestroy(grid, name="AttrI8l", rc=rc)
      write(failMsg, *) "Did not return ESMF_SUCCESS"
      write(name, *) "Destroying an ESMF_I8l Attribute on a Grid Test"
      call ESMF_Test((rc==ESMF_SUCCESS), name, failMsg, result, ESMF_SRCLINE)
      !------------------------------------------------------------------------
    ...
    !-------------------------------------------------------------------------
      ...
      !EX_UTest
      ! Destroy an Attribute on a Grid Test
      call ESMF_AttributeDestroy(grid, name="AttrR4", rc=rc)
      write(failMsg, *) "Did not return ESMF_SUCCESS"
      write(name, *) "Destroying an ESMF_R4 Attribute on a Grid Test"
      call ESMF_Test((rc==ESMF_SUCCESS), name, failMsg, result, ESMF_SRCLINE)
      !------------------------------------------------------------------------
    ...
    !-------------------------------------------------------------------------
      ...
      call ESMF_AttributeSet(grid, name="AttrR4l", count=count, &
      ...
      call ESMF_AttributeGet(grid, name="AttrR4l", count=count, &
      ...
      !EX_UTest
      ! Destroy an Attribute on a Grid Test
      call ESMF_AttributeDestroy(grid, name="AttrR4l", rc=rc)
      write(failMsg, *) "Did not return ESMF_SUCCESS"
      write(name, *) "Destroying an ESMF_R4l Attribute on a Grid Test"
      call ESMF_Test((rc==ESMF_SUCCESS), name, failMsg, result, ESMF_SRCLINE)
      !------------------------------------------------------------------------
    ...
    !-------------------------------------------------------------------------
      ...
      !EX_UTest
      ! Destroy an Attribute on a Grid Test
      call ESMF_AttributeDestroy(grid, name="AttrR8", rc=rc)
      write(failMsg, *) "Did not return ESMF_SUCCESS"
      write(name, *) "Destroying an ESMF_R8 Attribute on a Grid Test"
      call ESMF_Test((rc==ESMF_SUCCESS), name, failMsg, result, ESMF_SRCLINE)
      !------------------------------------------------------------------------
    ...
    !-------------------------------------------------------------------------
      ...
      call ESMF_AttributeSet(grid, name="AttrR8l", count=count, &
      ...
      call ESMF_AttributeGet(grid, name="AttrR8l", count=count, &
      ...
      !EX_UTest
      ! Destroy an Attribute on a Grid Test
      call ESMF_AttributeDestroy(grid, name="AttrR8l", rc=rc)
      write(failMsg, *) "Did not return ESMF_SUCCESS"
      write(name, *) "Destroying an ESMF_R8l Attribute on a Grid Test"
      call ESMF_Test((rc==ESMF_SUCCESS), name, failMsg, result, ESMF_SRCLINE)
      !------------------------------------------------------------------------
    ...
    !-------------------------------------------------------------------------
      ...
      !EX_UTest
      ! Destroy an Attribute on a Grid Test
      call ESMF_AttributeDestroy(grid, name=attrname, rc=rc)
      write(failMsg, *) "Did not return ESMF_SUCCESS"
      write(name, *) "Destroying a char Attribute on a Grid Test"
      call ESMF_Test((rc==ESMF_SUCCESS), name, failMsg, result, ESMF_SRCLINE)
      !------------------------------------------------------------------------
    ...
    !-------------------------------------------------------------------------
      ...
      call ESMF_AttributeSet(grid, name="Charl", count=count, &
      ...
      call ESMF_AttributeGet(grid, name="Charl", count=count, &
      ...
      !EX_UTest
      ! Destroy an Attribute on a Grid Test
      call ESMF_AttributeDestroy(grid, name="Charl", rc=rc)
      write(failMsg, *) "Did not return ESMF_SUCCESS"
      write(name, *) "Destroying a Character list Attribute on a Grid Test"
      call ESMF_Test((rc==ESMF_SUCCESS), name, failMsg, result, ESMF_SRCLINE)
      !------------------------------------------------------------------------

      !EX_UTest
      ! Destroy an Attribute on a Grid Test, again
      call ESMF_AttributeDestroy(grid, name="Charl", rc=rc)
      write(failMsg, *) "Did not return ESMF_SUCCESS"
      write(name, *) "Destroying a Character list Attribute on a Grid Test, again"
      call ESMF_Test((rc/=ESMF_SUCCESS), name, failMsg, result, ESMF_SRCLINE)
      !------------------------------------------------------------------------
    ...
    !-------------------------------------------------------------------------
      ...
      call ESMF_AttributeDestroy(grid, name=attrname, rc=rc)
      write(failMsg, *) "Did not return ESMF_SUCCESS"
      write(name, *) "Destroying a logical Attribute on a Grid Test"
      call ESMF_Test((rc==ESMF_SUCCESS), name, failMsg, result, ESMF_SRCLINE)
      !------------------------------------------------------------------------
    ...
    !-------------------------------------------------------------------------
      ...
      call ESMF_AttributeDestroy(grid, name=attrname, rc=rc)
      write(failMsg, *) "Did not return ESMF_SUCCESS"
      write(name, *) "Destroying a logical list Attribute on a Grid Test"
      call ESMF_Test((rc==ESMF_SUCCESS), name, failMsg, result, ESMF_SRCLINE)
      !------------------------------------------------------------------------
    ...
    !-------------------------------------------------------------------------
      ...
      !EX_UTest
      ! Destroy an Attribute in an Attribute package on a Grid Test
      call ESMF_AttributeDestroy(grid, name=attrname, convention=conv, purpose=purp, rc=rc)
      write(failMsg, *) "Did not return ESMF_SUCCESS"
      write(name, *) "Destroying an Attribute in an Attribute package on a Grid Test"
      call ESMF_Test((rc==ESMF_SUCCESS), name, failMsg, result, ESMF_SRCLINE)
      !------------------------------------------------------------------------
      ...
      call ESMF_AttributeWrite(grid, convention=conv, purpose=purp, rc=rc)
      write(failMsg, *) "Did not return ESMF_SUCCESS"
    ...
    !-------------------------------------------------------------------------
      ...
      call ESMF_AttributeSet(grid, name="ESMF_I4", value=inI4, &
        convention=conv, purpose=purp, rc=rc)
      write(failMsg, *) "Did not return ESMF_SUCCESS"
      write(name, *) "Setting an ESMF_I4 Attribute in an Attribute package on a Grid Test"
      call ESMF_Test((rc==ESMF_SUCCESS), name, failMsg, result, ESMF_SRCLINE)
      !------------------------------------------------------------------------

      count = 3
      !EX_UTest
      ! Set an ESMF_I4list Attribute in an Attribute package on a Grid Test
      call ESMF_AttributeSet(grid, name="ESMF_I4list", count=count, valueList=inI4l, &
        convention=conv, purpose=purp, rc=rc)
      write(failMsg, *) "Did not return ESMF_SUCCESS"
      write(name, *) "Setting an ESMF_I4list Attribute in an Attribute package on a Grid Test"
      call ESMF_Test((rc==ESMF_SUCCESS), name, failMsg, result, ESMF_SRCLINE)
      !------------------------------------------------------------------------

      !EX_UTest
      ! Set an ESMF_I8 Attribute in an Attribute package on a Grid Test
      call ESMF_AttributeSet(grid, name="ESMF_I8", value=inI8, &
        convention=conv, purpose=purp, rc=rc)
      write(failMsg, *) "Did not return ESMF_SUCCESS"
      write(name, *) "Setting an ESMF_I8 Attribute in an Attribute package on a Grid Test"
      call ESMF_Test((rc==ESMF_SUCCESS), name, failMsg, result, ESMF_SRCLINE)
      !------------------------------------------------------------------------

      count = 3
      !EX_UTest
      ! Set an ESMF_I8list Attribute in an Attribute package on a Grid Test
      call ESMF_AttributeSet(grid, name="ESMF_I8list", count=count, valueList=inI8l, &
        convention=conv, purpose=purp, rc=rc)
      write(failMsg, *) "Did not return ESMF_SUCCESS"
      write(name, *) "Setting an ESMF_I8list Attribute in an Attribute package on a Grid Test"
      call ESMF_Test((rc==ESMF_SUCCESS), name, failMsg, result, ESMF_SRCLINE)
      !------------------------------------------------------------------------

      !EX_UTest
      ! Set an ESMF_R4 Attribute in an Attribute package on a Grid Test
      call ESMF_AttributeSet(grid, name="ESMF_R4", value=inR4, &
        convention=conv, purpose=purp, rc=rc)
      write(failMsg, *) "Did not return ESMF_SUCCESS"
      write(name, *) "Setting an ESMF_R4 Attribute in an Attribute package on a Grid Test"
      call ESMF_Test((rc==ESMF_SUCCESS), name, failMsg, result, ESMF_SRCLINE)
      !------------------------------------------------------------------------

      count = 3
      !EX_UTest
      ! Set an ESMF_R4list Attribute in an Attribute package on a Grid Test
      call ESMF_AttributeSet(grid, name="ESMF_R4list", count=count, valueList=inR4l, &
        convention=conv, purpose=purp, rc=rc)
      write(failMsg, *) "Did not return ESMF_SUCCESS"
      write(name, *) "Setting an ESMF_R4list Attribute in an Attribute package on a Grid Test"
      call ESMF_Test((rc==ESMF_SUCCESS), name, failMsg, result, ESMF_SRCLINE)
      !------------------------------------------------------------------------

      !EX_UTest
      ! Set an ESMF_R8 Attribute in an Attribute package on a Grid Test
      call ESMF_AttributeSet(grid, name="ESMF_R8", value=inR8, &
        convention=conv, purpose=purp, rc=rc)
      write(failMsg, *) "Did not return ESMF_SUCCESS"
      write(name, *) "Setting an ESMF_R8 Attribute in an Attribute package on a Grid Test"
      call ESMF_Test((rc==ESMF_SUCCESS), name, failMsg, result, ESMF_SRCLINE)
      !------------------------------------------------------------------------

      count = 3
      !EX_UTest
      ! Set an ESMF_R8list Attribute in an Attribute package on a Grid Test
      call ESMF_AttributeSet(grid, name="ESMF_R8list", count=count, valueList=inR8l, &
        convention=conv, purpose=purp, rc=rc)
      write(failMsg, *) "Did not return ESMF_SUCCESS"
      write(name, *) "Setting an ESMF_R8list Attribute in an Attribute package on a Grid Test"
      call ESMF_Test((rc==ESMF_SUCCESS), name, failMsg, result, ESMF_SRCLINE)
      !------------------------------------------------------------------------

      !EX_UTest
      ! Set an ESMF_R8 Attribute in an Attribute package on a Grid Test
      call ESMF_AttributeSet(grid, name="Character_", value=attrvalue, &
        convention=conv, purpose=purp, rc=rc)
      write(failMsg, *) "Did not return ESMF_SUCCESS"
      ...
      !EX_UTest
      ! Destroy an Attribute in an Attribute package on a Grid Test, again
      call ESMF_AttributeDestroy(grid, name=attrname, convention=conv, purpose=purp, rc=rc)
      write(failMsg, *) "Did not return ESMF_SUCCESS"
      write(name, *) "Destroying an Attribute in an Attribute package on a Grid Test, again"
      call ESMF_Test((rc/=ESMF_SUCCESS), name, failMsg, result, ESMF_SRCLINE)
      !------------------------------------------------------------------------
