! $Id: ESMF_GridComp.F90,v 1.45 2004/05/21 09:31:13 nscollins Exp $
!
...
!EOPI
        ...
        if (ESMF_LogMsgFoundAllocError(status, "compclass", &
...
!  \begin{description}
!   \item[vm]
!    VM of the component out of which this {\tt ESMF\_GridCompCreate}
!    call is issued.
...
!     \begin{description}
!     \item[gridcomp]
!      Destroy contents of this
!      {\tt ESMF\_GridComp} gridded component object.
!     \item[{[rc]}]
!      Return code; equals {\tt ESMF\_SUCCESS} if there are no errors.
!     \end{description}
!
!EOP
        ...
        if (ESMF_LogMsgFoundAllocError(status, "compclass dealloc", &
                                       ESMF_CONTEXT, rc)) return
...
!  \begin{description}
!   \item[gridcomp]
!    The {\tt ESMF\_GridComp} gridded component object to query.
!   \item[{[name]}]
!    GridComp name.
!   \item[{[gridcomptype]}]
!    GridComp Model Type, where model includes {\tt ESMF\_ATM, ESMF\_LAND,
...
!  \begin{description}
!   \item[gridcomp]
!    The {\tt ESMF\_GridComp} gridded component object to set value for.
!   \item[{[name]}]
!    GridComp name.
!   \item[{[gridcomptype]}]
!    GridComp Model Type, where model includes {\tt ESMF\_ATM, ESMF\_LAND,
...
!  \begin{description}
!   \item[gridcomp]
!    The {\tt ESMF\_GridComp} gridded component object to validate.
!   \item[{[options]}]  
...
!  \begin{description}
!   \item[gridcomp]
!    The {\tt ESMF\_GridComp} gridded component object to print.
!   \item[{[options]}]
!    Print options.
!   \item[{[rc]}]
!    Return code; equals {\tt ESMF\_SUCCESS} if there are no errors.
...
!  \begin{description}
!   \item[gridcomp]
!    The {\tt ESMF\_GridComp} to call Initialization routine for.
!   \item[{[importState]}]  
...
!  \begin{description}
!   \item[gridcomp]
!    The {\tt ESMF\_GridComp} to call Run routine for.
!   \item[{[importState]}]  
!    Import data for run.
!   \item[{[exportState]}]  
!    Export data for run.
...
!  \begin{description}
!   \item[gridcomp]
!    The {\tt ESMF\_GridComp} to call WriteRestart routine for.
!   \item[{[iospec]}]  
!    I/O options.
!   \item[{[clock]}]  
!    External clock for passing in time information.
...
!  \begin{description}
!   \item[gridcomp]
!    The {\tt ESMF\_GridComp} to call ReadRestart routine for.
!   \item[{[iospec]}]  
!    I/O options.
!   \item[{[clock]}]  
!    External clock for passing in time information.
...
!     \begin{description}
!     \item[gridcomp] 
!      An {\tt ESMF\_GridComp} gridded component object.
!     \item[{[max]}] 
