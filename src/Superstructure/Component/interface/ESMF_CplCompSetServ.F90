! $Id: ESMF_CplCompSetServ.F90,v 1.1 2004/03/19 20:01:42 cdeluca Exp $
!
...
!
! !DESCRIPTION:
!  Available to be called by a cplcomp at any time after 
...
!  When running multiple instantiations of a cplcomp, for example during
...
!  \begin{description}
!   \item[cplcomp]
!    CplComp object returned from a {\tt ESMF\_CplCompCreate} call.
!   \item[dataPointer]
...
!
! !DESCRIPTION:
!  Intended to be called by a cplcomp during the Registration process.
!  A cplcomp calls SetEntryPoint for each of the predefined Init,
...
!  the Initialize, Run, and Finalize routines for the subcplcomps.
!    
!  The arguments are:
!  \begin{description}
!   \item[cplcomp]
...
!   \item[subroutineName]
!    The name of the CplComp's subroutine to be associated with the
!    subroutineType (e.g. Initialize).   This subroutine does not have to be
!    public to the module.
!   \item[{[phase]}] 
!    For CplComps which need to initialize or run or finalize in mutiple
...
!    parameter {\tt ESMF\_SINGLEPHASE}.   The CplComp writer must document
!    the requirements of the CplComp for how and when the multiple phases
...
!
! !DESCRIPTION:
!  Available to be called by a cplcomp at any time, but expected to be
...
!
! !DESCRIPTION:
!  Call a Coupler {\tt ESMF\_CplComp}'s SetServices registration routine.  
!  The parent CplComp must first create a subcplcomp and then
!  call this routine, giving it the cplcomp derived type
!  returned from the create, plus the public, well-known, subroutine name
!  that is the registration routine for that cplcomp.  This name must be
!  documented by the subcplcomp provider.
!
!  After this subroutine returns, the framework now knows how to call
!  the Initialize, Run, and Finalize routines for the subcplcomps.
!    
...
!    CplComp object returned from a {\tt ESMF\_CplCompCreate} call.
!   \item[subroutineName]
!    The public name of the CplComp's {\tt SetServices} call.  
!    A cplcomp writer must provide this information.
!   \item[{[rc]}] 
!    Return code; equals {\tt ESMF\_SUCCESS} if there are no errors.
!   \end{description}
!
