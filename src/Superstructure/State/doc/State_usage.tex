% $Id: State_usage.tex,v 1.8 2007/06/22 23:21:49 cdeluca Exp $
%
...
A InternGridded Component generally has one associated import 
State and one export State.  Generally the States 
associated with a InternGridded Component will be created by 
the InternGridded Component's parent component.
In many cases, the States will be created containing 
no data.  Both the empty States and the
newly created InternGridded Component are passed
by the parent component into the InternGridded Component's initialize 
...
into the InternGridded Component's initialize method, Field,
...
that another InternGridded Component requires, and passed out
of the Coupler Component as its export State.  It can then
be passed into the run method of a recipient InternGridded Component
...
the user can make the InternGridded Component's initialize method 
clever enough to not allocate the data for that item at all
and not compute it within the InternGridded Component code.
