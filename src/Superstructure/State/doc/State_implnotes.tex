% $Id: State_implnotes.tex,v 1.9 2007/06/22 23:21:49 cdeluca Exp $
%
...
it needs to know about any remote PET (for example, the interngrid
distribute method can compute the decomposition of the interngrid on
...
For example, the FieldExcl system test creates 2 interngridded components
on separate subsets of PETs.  They use the option of mapping
particular, non-monotonic PETs to DEs.  The following figures 
illustrate how the DEs are mapped in each of the interngridded components
in that test:

\begin{center}
\begin{figure}
\scalebox{0.9}{\includegraphics{Excl_src_interngrid}}
\caption{The mapping of PETs (processors) to DEs (data)
in the source interngrid created by {\tt user\_model1.F90}
...
\begin{center}
\begin{figure}
\scalebox{0.9}{\includegraphics{Excl_dst_interngrid}}
\caption{The mapping of PETs (processors) to DEs (data)
in the destination interngrid created by {\tt user\_model2.F90}
in the FieldExcl system test.}
...
with the object, but creates a slightly different InternGrid object,
called a Proxy InternGrid. These PETs contain no local data, so the
Array object is empty, and the DELayout for the InternGrid is like this:

\begin{center}
\begin{figure}
\scalebox{0.9}{\includegraphics{Excl_src_interngrid_cpl}}
\caption{The mapping of PETs (processors) to DEs (data)
in the source interngrid after the reconcile call in {\tt user\_coupler.F90}
in the FieldExcl system test.}
\label{fig:excl_source}
\end{figure}
\end{center}
...
\begin{center}
\begin{figure}
\scalebox{0.9}{\includegraphics{Excl_dst_interngrid_cpl}}
\caption{The mapping of PETs (processors) to DEs (data)
in the destination interngrid after the reconcile call in {\tt user\_coupler.F90}
in the FieldExcl system test.}
\label{fig:excl_destination}
\end{figure}
\end{center}
